...
static volatile uint_fast16_t usb_cdc_control_state [INTERFACE_count];	
	...
	static USBALIGN_BEGIN uint8_t cdc1buffout [VIRTUAL_COM_PORT_DATA_SIZE] USBALIGN_END;
	static USBALIGN_BEGIN uint8_t cdc1buffin [VIRTUAL_COM_PORT_DATA_SIZE] USBALIGN_END;
	static uint_fast16_t cdc1buffinlevel;

	static USBALIGN_BEGIN uint8_t cdc2buffout [VIRTUAL_COM_PORT_DATA_SIZE] USBALIGN_END;
...
static void usbd_classDeInit(USBD_HandleTypeDef *pdev, uint_fast8_t cfgidx);
...
static void usb0_function_GetInterface(PCD_TypeDef * const Instance, uint_fast8_t ReqTypeRecip, uint_fast16_t ReqValue, uint_fast16_t ReqIndex, uint_fast16_t ReqLength)
{
	...
	static USBALIGN_BEGIN uint8_t buff [INTERFACE_count] [1] USBALIGN_END;
...
static void usbdFunctionReq_seq1(PCD_TypeDef * const Instance, uint_fast8_t ReqType, uint_fast8_t ReqRequest, uint_fast16_t ReqValue, uint_fast16_t ReqIndex, uint_fast16_t ReqLength)
{
	...
	*/
	...
	static USBALIGN_BEGIN uint8_t buff [INTERFACE_count] [64] USBALIGN_END;
	switch (interfacev)
	{
#if WITHUSBCDC
	case INTERFACE_CDC_CONTROL_3a:	// CDC interface
	case INTERFACE_CDC_CONTROL_3b:	// CDC interface
		{
			...
			case CDC_GET_LINE_CODING:
				...
				control_transmit(Instance, buff [interfacev], ulmin16(7, ReqLength));
				return;
	...
	default:
		...
		memset(buff [interfacev], 0, ulmin16(ARRAY_SIZE(buff [interfacev]), ReqLength));
		control_transmit(Instance, buff [interfacev], ulmin16(ARRAY_SIZE(buff [interfacev]), ReqLength));
		return;
	}
...
static void usbd_classDeInit(USBD_HandleTypeDef *pdev, uint_fast8_t cfgidx)
{
	//debug_printf_P(PSTR("usbd_classDeInit\n"));
	...
	//debug_printf_P(PSTR("usbd_classDeInit done\n"));
}
...
3. Set up the Data FIFO RAM for each of the FIFOs – 
  ...
  */
	...
	const uint_fast16_t size4dummy = 4;//0x10;
	last4 -= size4dummy;
	...
	debug_printf_P(PSTR("usbd_fifo_initialize2: 4*(full4-last4)=%u\n"), 4 * (full4 - last4));
#if WITHUSBUAC3
	{
	...
	debug_printf_P(PSTR("usbd_fifo_initialize3: 4*(full4-last4)=%u\n"), 4 * (full4 - last4));
#endif /* WITHUSBUAC3 */
#endif /* WITHUSBUAC */
	...
	for (i = 0; i < WITHUSBHWCDC_N; ++ i)
	{
		...
		else
		{
			...
			const uint_fast16_t size4 = ncdcindatapackets * (size2buff4(VIRTUAL_COM_PORT_DATA_SIZE) + add3tx);
			ASSERT(last4 >= size4);
			last4 -= size4;
			instance->DIEPTXF [pipe - 1] = usbd_makeTXFSIZ(last4, size4);
			instance->DIEPTXF [pipeint - 1] = usbd_makeTXFSIZ(last4dummy, size4dummy);
		}
	debug_printf_P(PSTR("usbd_fifo_initialize4: 4*(full4-last4)=%u\n"), 4 * (full4 - last4));
	}
	...
	debug_printf_P(PSTR("usbd_fifo_initialize5: 4*(full4-last4)=%u\n"), 4 * (full4 - last4));
#endif /* WITHUSBCDCEEM */

#if WITHUSBCDCECM
	debug_printf_P(PSTR("usbd_fifo_initialize6: 4*(full4-last4)=%u\n"), 4 * (full4 - last4));
#endif /* WITHUSBCDCECM */
	...
	debug_printf_P(PSTR("usbd_fifo_initialize7: 4*(full4-last4)=%u\n"), 4 * (full4 - last4));
#endif /* WITHUSBHID */
	...
	/* control endpoint RX FIFO */
	{
		...
		debug_printf_P(PSTR("usbd_fifo_initialize10: size4*4=%u bytes, 4*(full4-last4)=%u bytes\n"), 4 * size4, 4 * (full4 - last4));
		ASSERT(last4 >= size4);
...
USBD_StatusTypeDef  USBD_Stop   (USBD_HandleTypeDef *pdev)
{
	...
	usbd_classDeInit(pdev, pdev->dev_config [0]);
	#if 0
	#endif
	if (pdev->pClass != NULL)
		pdev->pClass->DeInit(pdev, pdev->dev_config [0]);  
...
static USBD_StatusTypeDef USBD_SetClassConfig(USBD_HandleTypeDef  *pdev, uint8_t cfgidx)
{
	...
	//debug_printf_P(PSTR("USBD_SetClassConfig: cfgidx=%d\n"), cfgidx);
	memset(altinterfaces, 0, sizeof altinterfaces);
 	...
 	for (offset = 0; offset < WITHUSBHWCDC_N; ++ offset)
	{
	   USBD_LL_OpenEP(pdev,
	     ...
	     /* cdc Open EP OUT */
		USBD_LL_OpenEP(pdev, USBD_EP_CDC_OUT + offset, USBD_EP_TYPE_BULK, VIRTUAL_COM_PORT_DATA_SIZE);
			...
			case INTERFACE_CDC_CONTROL_3b:	// CDC interface
				{
					...
					case CDC_GET_LINE_CODING:
						...
						USBD_CtlSendData(pdev, buff [interfacev], ulmin16(7, req->wLength));
						break;
...
static void USBD_SetFeature(USBD_HandleTypeDef *pdev, 
                            USBD_SetupReqTypedef *req)
{
	//debug_printf_P(PSTR("USBD_SetFeature:\n"));
...
USBD_StatusTypeDef USBD_LL_DataOutStage(USBD_HandleTypeDef *pdev, uint8_t epnum, uint8_t *pdata)
{
		...
		if (pdev->ep0_state == USBD_EP0_DATA_OUT)
		{
			...
			else
			{
				if ((pdev->dev_state == USBD_STATE_CONFIGURED))
				{
					//
					//
					const uint_fast8_t interfacev = LO_BYTE(pdev->request.wIndex);
					switch (interfacev)
					{
#if WITHUSBCDC
					case INTERFACE_CDC_CONTROL_3a:	// CDC interface
					case INTERFACE_CDC_CONTROL_3b:	// CDC interface
						...
						case CDC_SET_LINE_CODING:
							dwDTERate [interfacev] = 
								...
								//debug_printf_P(PSTR("USBD_LL_DataOutStage: CDC_SET_LINE_CODING: interfacev=%u, dwDTERate=%lu, bits=%u (size=%u)\n"), interfacev, dwDTERate [interfacev], ep0databuffout [6], USBD_LL_GetRxDataSize(pdev, epnum));
							break;

						default:
							debug_printf_P(PSTR("USBD_LL_DataOutStage: xxx1: interfacev=%u\n"), interfacev);
							break;
						}
						break;
#endif /* WITHUSBCDC */
	...
	else if (pdev->dev_state == USBD_STATE_CONFIGURED)
	{
		...
		case USBD_EP_CDC_OUT:
			...
			USBD_LL_PrepareReceive(pdev, USB_ENDPOINT_OUT(epnum), cdc1buffout, VIRTUAL_COM_PORT_DATA_SIZE);      
			break;
		...
		case USBD_EP_CDC_OUTb:
			...
			/* Prepare Out endpoint to receive next cdc data packet */
			USBD_LL_PrepareReceive(pdev, USB_ENDPOINT_OUT(epnum), cdc2buffout, VIRTUAL_COM_PORT_DATA_SIZE);      
			break;
#endif /* WITHUSBCDC */
		...
		case USBD_EP_CDCEEM_OUT:
			cdceemout_buffer_save(cdceemoutbuff, USBD_LL_GetRxDataSize(pdev, epnum));
...
#if 0
#endif
	if ((pdev->pClass != NULL) && (pdev->pClass->DeInit != NULL) && (pdev->pClassData != NULL)) 
		pdev->pClass->DeInit(pdev, pdev->dev_config [0]);  
...
static const USBD_ClassTypeDef  USBD_CDC = 
{
  USBD_CDC_Init,
  USBD_CDC_DeInit,
  USBD_CDC_Setup,
...
void HAL_PCD_MspInit(PCD_HandleTypeDef *hpcd)
{
	...
	if (hpcd->Instance == USB_OTG_HS)
	{
		...
		else
		{
			...
			debug_printf_P(PSTR("hardware_usbd_initialize: HS without ULPI\n"));
			RCC->AHB1ENR |= RCC_AHB1ENR_OTGHSEN;	/* USB/OTG HS  */
			(void) RCC->AHB1ENR;
		}
...
static USBD_StatusTypeDef USBD_DeInit(USBD_HandleTypeDef *pdev)
{
	...
	#if 0
	#endif
	if (pdev != NULL && pdev->pClass != NULL && pdev->pClass->DeInit != NULL)
		pdev->pClass->DeInit(pdev, pdev->dev_config [0]);  
...
static void hardware_usbd_initialize(void)
{
	...
	//debug_printf_P(PSTR("hardware_usbd_initialize.\n"));
	// was: MX_USB_DEVICE_Init();
	/* Init Device Library,Add Supported Class and Start the library*/
	if (ifhs != 0)
	{
		USBD_HS_Init(& hpcd_USB_OTG, & hUsbDevice);
	}
	else
	{
		USBD_FS_Init(& hpcd_USB_OTG, & hUsbDevice);
	}
	////USBD_RegisterClass(&hUsbDevice, &USBD_AUDIO);
	////USBD_RegisterClass(&hUsbDevice, &USBD_CDC);

	////USBD_AUDIO_RegisterInterface(&hUsbDevice, &USBD_AUDIO_fops_FS);

	////usbd_endpoints_activate(& hpcd_USB_OTG);	// for tests

	//USBD_Start(& hUsbDevice);	// moved to 

	//debug_printf_P(PSTR("hardware_usbd_initialize done\n"));
}
