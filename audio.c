	...
	)
{
	...
	case DSPCTL_MODE_TX_FREEDV:
		...
		case BOARD_TXAUDIO_2TONE:
			...
			// return audio sample in range [- txlevelfence.. + txlevelfence]
			return injectsubtone(get_dualtonefloat() * txlevelfence, ctcss);		// источник сигнала - двухтональный генератор для настройки
...
void dsp_initialize(void)
{
	...
	txlevelfenceINT = txlevelfence;	// Для источника шума
