...
static const struct fir_zoom_param FirZoomFFTDecimate [BOARD_FFTZOOM_POW2MAX] =
{
	// x2 zoom lowpass
	{
		.zoom = 2,
...
static const float32_t * const iir_coeffs [BOARD_FFTZOOM_POW2MAX] =
{
	coeffs_x2, // 2
#if BOARD_FFTZOOM_POW2MAX > 1
	coeffs_x4, // 4
	...
	)
{
	...
	arm_biquad_cascade_df1_init_f32(& iir_config, FFTZOOM_IIR_STAGES, iir_coeffs [zoompow2 - 1], iir_state);
	...
	VERIFY(ARM_MATH_SUCCESS == arm_fir_decimate_init_f32(& fir_config,
						...
						FirZoomFFTDecimate [zoompow2 - 1].zoom,          // Decimation factor
