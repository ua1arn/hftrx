...
catch {set coef_files [glob -nocomplain fir_10ex_2ch_coef*.txt]} error_msg
puts "$error_msg"
  
# The top-level in HDL type "VERILOG"
set ipfs_ext vo
if {[file exists fir_10ex_2ch.vo]} {
	set hdl_ext v
} else {
	puts "Warning: Could not find fir_10ex_2ch.$ipfs_ext!"
}
...
set_global_assignment -name EDA_TEST_BENCH_MODULE_NAME work.tb_fir_10ex_2ch -section_id tb
set_global_assignment -name EDA_TEST_BENCH_GATE_LEVEL_NETLIST_LIBRARY work -section_id tb

# IPFS file
set_global_assignment -name EDA_IPFS_FILE fir_10ex_2ch.$ipfs_ext -section_id eda_simulation -library work
...
set_global_assignment -name EDA_TEST_BENCH_FILE fir_10ex_2ch_input.txt -section_id tb -library work

set_global_assignment -name EDA_TEST_BENCH_FILE tb_fir_10ex_2ch.vhd -section_id tb -library work
