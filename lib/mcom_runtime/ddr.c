...
#define CPLL_VALUE (SEL_CPLL_REG & 0xFF)
...
static void set_nt5cb256m8fn_di_cfg()
{
...
void set_ddr_freq(unsigned int MHz)
{
    ...
    // Set CPLL
    SEL_CPLL_REG = MHZ_TO_SEL(MHz);
    while (!(SEL_CPLL_REG & PLL_LOCK_BIT))
        ;
}
...
int DDR_INIT(unsigned int reset_type, void *mem_cfg_0, void *mem_cfg_1)
{
    ...
    sys_t sys;
    sys.CMCTR = (cmctr_t *)(CMCTR_BASE);
    sys.PMCTR = (pmctr_t *)(PMCTR_BASE);
    sys.DDR_REMAP = (unsigned int *)(SMCTR_BASE + 0xc);
    sys.BASE_ADR[0] = DDR0_BA;
    sys.BASE_ADR[1] = DDR1_BA;

    while ((sys.PMCTR->CORE_PWR_STATUS & 1)) {
    }
                       ...
                       const unsigned reset_type)
{
    ...
    ddr_ret = sys->PMCTR->DDR_PIN_RET;

    if (ctl_enabled[0])
        ddr_ret = ddr_ret & (2);
    if (ctl_enabled[1])
        ddr_ret = ddr_ret & (1);
    sys->PMCTR->DDR_PIN_RET = ddr_ret;
    ...
    for (j = 0; j < 2; j++) {
        ...
        sys->CMCTR->GATE_CORE_CTR = sys->CMCTR->GATE_CORE_CTR | (1 << (j + 1));
