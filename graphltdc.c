...
arm_hardware_ltdc_initialize(void)
{
	...
	SETREG32(& VDC50.TCON_TIM, 11, 16, WIDTH);	// TCON_HALF
	...
	SETREG32(& VDC50.SC0_SCL0_UPDATE, 1, 8, 1);		// what is a bit? SC0_SCL0_UPDATE	SYNC Control Register Update
...
static void LCD_LayerInit(
	...
	unsigned scale_h,
	unsigned pixelsize	// дл€ расчета размера строки в байтах
	)
{
	...
	LTDC_Layer_InitStruct.LTDC_BlendingFactor_1 = LTDC_BlendingFactor1_CA; // умножитель дл€ пиксел€ из текущего сло€
	LTDC_Layer_InitStruct.LTDC_BlendingFactor_2 = LTDC_BlendingFactor2_CA; // умножитель дл€ пиксел€ из расположенного ниже сло€
...
{
	// преобразование из упакованного пиксел€ RGB565 по правилам pfc LTDC
	// в требующийс€ RGB888
