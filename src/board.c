...
prog_ctrlreg(uint_fast8_t plane)
{
	// registers chain control register
	{
		...
		RBBIT(0010, glob_poweron);						// D0: 1 - снять SHUTDOWN с усилителя LM4950/PAM8406
...
static void prog_ctrlreg(uint_fast8_t plane)
{
	...
	RBVAL(021, glob_tx ? 0 : (1U << glob_bandf) >> 1, 7); // бит выбора диапазонного фильтра приёмника

	RBVAL(012, glob_attvalue, 4);	// PE4302 attenuator
	RBBIT(011, glob_attvalue >> 4); // PE4302 attenuator MSB
	RBBIT(010, txgated); 			// PA enable

	RBBIT(007, glob_bandf == 0);	// ~BAND0_RELAY
	RBBIT(005, glob_bandf == 0); 	// ~BAND0_RELAY_OUT
	RBBIT(004, ! xvrtr && glob_tx);	// D1: TX ANT relay
	RBVAL(001, ~ ((! xvrtr && txgated) ? powerxlat [glob_stage1level] : HARDWARE_OPA2674I_SHUTDOWN), 2);	// A1..A0 of OPA2674I-14D
...
void
board_set_mla_paramC(uint_fast16_t n)
{
	if (glob_mla_paramC != n)
	{
		glob_mla_paramC = n;
		board_ctlreg1changed();
	}
...
void board_reload_fir(uint_fast8_t ifir, const int32_t * const k, const FLOAT_t * const kf, unsigned Ntap, unsigned CWidth)
{
	...
	for (i = 0; i <= iHalfLen; ++ i)
	{
		...
		board_reload_fir_artix7_p1(targetfpga1, 13, coeff << bits);
	}
...
static void ua1cei_magloop_send(void)
{
	...
	unsigned len = local_snprintf_P(state, ARRAY_SIZE(state),
			...
			(unsigned) glob_mla_paramC,
			(unsigned) ! glob_tuner_bypass
		);
	nmea_send(state, len);
...
static LCLSPINLOCK_t gpreilock = IRQLSPINLOCK_INIT;
...
{
	return !! (vstatus & (FPGA_STATUS_ADC1OVF | FPGA_STATUS_ADC2OVF | FPGA_STATUS_ADC3OVF | FPGA_STATUS_ADC4OVF));
}
