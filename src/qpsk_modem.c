...
void modem_test(void)
{
	...
	static const uint8_t txarray [] = { 0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0x55, 0x55,  0xAA, 0xAA, 0xAA, 0xAA, 0xDE, 0xAD, 0xBE, 0xEF, 0x55, 0x55, 0x55,  };
	int n = app_modem_execute_transmission(txarray, ARRAY_SIZE(txarray));
	PRINTF("n=%d samples\n", n);
	TP();
	// Сдвигаем указатель на точку, где фильтр уже "раскачался" и выдает чистую фазу
	FLOAT_t *stable_rx_samples = tx_hardware_output_io + (MODEM_RRC_TAPS / 2);
	app_modem_rx_dma_callback_isr(stable_rx_samples);
