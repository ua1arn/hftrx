...
void hightests(void)
{
...
#if 0
	{
		...
		{
			GIC_SendSGI(int_id, 1U << core, 0x00);	// other CORE, filer=0
			local_delay_ms(250);

		}
		arm_hardware_disable_handler(int_id);
		PRINTF("IPC interrupte demo done\n");
	}
#endif
#if 0
	{
...
//			PRINTHEX32(* TIMESTAMP_CTRL);
			const uint64_t t0 = __get_CNTPCT();
			local_delay_ms(1000);
			const uint64_t t1 = __get_CNTPCT();
			unsigned dt = t1 - t0;
			PRINTF("t0=0x%" PRIX64 ", dt=%u\n", t0, dt);
		}
	}
#endif
