...
static uint_fast16_t encodeMaxPacketSize(int highspeed, uint_fast32_t size)
{
...
static unsigned UAC2_EP_Desc_OUT48(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, uint_fast8_t bEndpointAddress, const desc_options_t * opts)
{
	const uint_fast8_t length = 7;
	ASSERT(maxsize >= length);
	if (maxsize < length)
	...
	if (fill != 0 && buff != NULL)
	{
		const uint_fast16_t wMaxPacketSize = encodeMaxPacketSize(opts->hsdesc, usbd_getuacoutmaxpacket());
		// Вызов для заполнения, а не только для проверки занимаемого места в буфере
		* buff ++ = length;						  /* bLength */
		...
		* buff ++ = opts->hsdesc ? HSINTERVAL_AUDIO48 : FSINTERVAL_AUDIO48;    /* bInterval */
		/* 7 byte*/
	}
	return length;
}

//static unsigned UAC2_EP_Feedback_Desc_OUT48(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, uint_fast8_t bEndpointAddress, const desc_options_t * opts)
//{
//	const uint_fast8_t length = 7;
//	ASSERT(maxsize >= length);
//	if (maxsize < length)
...
static unsigned UAC2_EP_Desc_IN48(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, const desc_options_t * opts, uint_fast8_t bEndpointAddress, uint8_t offset)
{
	const uint_fast8_t length = 7;
	ASSERT(maxsize >= length);
	if (maxsize < length)
...
/* USB Microphone Standard Endpoint Descriptor (CODE == 8)*/ //Standard AS Isochronous Audio Data Endpoint Descriptor
// Endpoint Descriptor 82 2 In, Isochronous, 125 us
static unsigned UAC2_EP_Desc_IN_RTS96(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, const desc_options_t * opts, uint_fast8_t bEndpointAddress, uint8_t offset)
{
	const uint_fast8_t length = 7;
	ASSERT(maxsize >= length);
	if (maxsize < length)
	...
	if (fill != 0 && buff != NULL)
	{
		...
		* buff ++ = LO_BYTE(wMaxPacketSize);        /* wMaxPacketSize */
		* buff ++ = HI_BYTE(wMaxPacketSize); 
		* buff ++ = opts->hsdesc ? HSINTERVAL_RTS96 : FSINTERVAL_RTS96;    /* bInterval */
	}
	return length;
}
...
/* USB Microphone Standard Endpoint Descriptor (CODE == 8)*/ //Standard AS Isochronous Audio Data Endpoint Descriptor
// Endpoint Descriptor 82 2 In, Isochronous, 125 us
static unsigned UAC2_EP_Desc_RTS192(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, const desc_options_t * opts, uint_fast8_t bEndpointAddress, uint8_t offset)
{
	const uint_fast8_t length = 7;
	ASSERT(maxsize >= length);
	if (maxsize < length)
...
/* USB Microphone Standard Endpoint Descriptor (CODE == 8)*/ //Standard AS Isochronous Audio Data Endpoint Descriptor
// Endpoint Descriptor 82 2 In, Isochronous, 125 us
static unsigned UAC1_EP_Desc_IN48(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, const desc_options_t * opts, uint_fast8_t bEndpointAddress, uint8_t offset)
{
	const uint_fast8_t length = 9;
	ASSERT(maxsize >= length);
	if (maxsize < length)
...
/* USB Microphone Standard Endpoint Descriptor (CODE == 8)*/ //Standard AS Isochronous Audio Data Endpoint Descriptor
// Endpoint Descriptor 82 2 In, Isochronous, 125 us
static unsigned UAC1_EP_Desc_RTS96(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, const desc_options_t * opts, uint_fast8_t bEndpointAddress, uint8_t offset)
{
	const uint_fast8_t length = 9;
	ASSERT(maxsize >= length);
	if (maxsize < length)
...
/* USB Microphone Standard Endpoint Descriptor (CODE == 8)*/ //Standard AS Isochronous Audio Data Endpoint Descriptor
// Endpoint Descriptor 82 2 In, Isochronous, 125 us
static unsigned UAC1_EP_Desc_RTS192(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, const desc_options_t * opts, uint_fast8_t bEndpointAddress, uint8_t offset)
{
	const uint_fast8_t length = 9;
	ASSERT(maxsize >= length);
	if (maxsize < length)
...
static unsigned UAC2_AS_InterfaceDescPCM(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, uint_fast8_t bTerminalLink, uint_fast32_t bmChannelConfig)
{
	...
	if (fill != 0 && buff != NULL)
	{
		const uint_fast32_t bmFormats = 1u << 0;	// D0 PCM
...
static unsigned UAC2_AS_InterfaceDescIEEE(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, uint_fast8_t bTerminalLink, uint_fast32_t bmChannelConfig)
{
	...
	if (fill != 0 && buff != NULL)
	{
		const uint_fast32_t bmFormats = 1u << 2;	// D2 IEEE_FLOAT
		const uint_fast8_t bmControls = 0x00;	// D1..0: Active Alternate Setting Control
		// Вызов для заполнения, а не только для проверки занимаемого места в буфере
		// Table 4-27: Class-Specific AS Interface Descriptor
		* buff ++ = length;						  /* bLength */
...
static unsigned fill_UAC2_INRTS_function(
	uint_fast8_t fill, uint8_t * p, unsigned maxsize,
	const desc_options_t * opts,
	uint_fast8_t offset	// UACOFFS_INRTS here
	)
{
...
static unsigned fill_UAC2_IN48_function(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts, uint_fast8_t offset)
{
	unsigned n = 0;
...
//			.bCSourceID = UACTEix(TERMINAL_ID_CLKMULTIPLIER, UACOFFS_IN48),
	...
	n += UAC2_AS_InterfaceDesc(fill, p + n, maxsize - n, mikeifv, UACINALT_AUDIO48, 1, offset, iInterfaceIN48);	/* INTERFACE_AUDIO_MIKE Interface Descriptor 2/1 Audio, 1 Endpoint, bAlternateSetting=0x01 */
...
static unsigned fill_UAC2_IN48_INRTS_function(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts, uint_fast8_t offset)
{
	uint_fast8_t ialt = 0;
	unsigned n = 0;
...
static unsigned fill_UAC2_OUT48_function(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts, uint_fast8_t offset)
{
	uint_fast8_t ialt = 0;
	unsigned n = 0;
...
static unsigned UAC1_AS_InterfaceDescPCM(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, uint_fast8_t bTerminalLink)
{
	const uint_fast8_t length = 7;
	ASSERT(maxsize >= length);
	if (maxsize < length)
...
static unsigned fill_UAC1_INRTS_function(
	uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts,
	uint_fast8_t offset	// UACOFFS_INRTS here
	)
{
	const uint_fast8_t rtscontrolifv = INTERFACE_AUDIO_CONTROL_RTS;
...
#if WITHRTS96
	// IN data flow: radio RX spectrum data
	n += UAC1_InterfaceDesc(fill, p + n, maxsize - n, rtsifv, UACINRTSALT_RTS96, 1, offset, iInterfaceINRTS);	/* INTERFACE_AUDIO_RTS_2 Interface Descriptor 2/1 Audio, 1 Endpoint, bAlternateSetting=0x01 */
	n += UAC1_AS_InterfaceDescPCM(fill, p + n, maxsize - n, terminalID);	/* USB Microphone Class-specific AS General Interface Descriptor (for output TERMINAL_UACIN48_UACINRTS) (CODE == 5) */
	n += UAC1_FormatTypeDesc_RTS96(fill, p + n, maxsize - n);		/* USB Microphone Type I Format Type Descriptor (CODE == 6) 48000 */
	n += UAC1_EP_Desc_RTS96(fill, p + n, maxsize - n, opts, epinrts, offset);	/* Endpoint Descriptor USBD_EP_AUDIO_IN In, Isochronous, 125 us */
...
#if WITHRTS192
	// IN data flow: radio RX spectrum data
	n += UAC1_InterfaceDesc(fill, p + n, maxsize - n, rtsifv, UACINRTSALT_RTS192, 1, offset, iInterfaceINRTS);	/* INTERFACE_AUDIO_RTS_2 Interface Descriptor 2/1 Audio, 1 Endpoint, bAlternateSetting=0x01 */
	n += UAC1_AS_InterfaceDescPCM(fill, p + n, maxsize - n, terminalID);	/* USB Microphone Class-specific AS General Interface Descriptor (for output TERMINAL_UACIN48_UACINRTS) (CODE == 5) */
	n += UAC1_FormatTypeDesc_RTS192(fill, p + n, maxsize - n);		/* USB Microphone Type I Format Type Descriptor (CODE == 6) 48000 */
	n += UAC1_EP_Desc_RTS192(fill, p + n, maxsize - n, opts, epinrts, offset);	/* Endpoint Descriptor USBD_EP_AUDIO_IN In, Isochronous, 125 us */
	n += UAC1_AS_EP_Desc(fill, p + n, maxsize - n);	/* USB Microphone Class-specific Isoc. Audio Data Endpoint Descriptor (CODE == 7) OK - подтверждено документацией*/
#endif /* WITHRTS192 */
		...
		uint_fast8_t offset)
{
	...
	n += UAC1_InterfaceDesc(fill, p + n, maxsize - n, mikeifv, UACINALT_NONE, 0, offset, iInterfaceIN48);	/* USB Microphone Standard AS Interface Descriptor (Alt. Set. 0) (CODE == 3) */ //zero-bandwidth interface

	// IN data flow: radio RX audio data
	n += UAC1_InterfaceDesc(fill, p + n, maxsize - n, mikeifv, UACINALT_AUDIO48, 1, offset, iInterfaceIN48);	/* INTERFACE_AUDIO_MIKE Interface Descriptor 2/1 Audio, 1 Endpoint, bAlternateSetting=0x01 */
	n += UAC1_AS_InterfaceDescPCM(fill, p + n, maxsize - n, terminalID);	/* USB Microphone Class-specific AS General Interface Descriptor (for output TERMINAL_UACIN48_UACINRTS) (CODE == 5) */
	n += UAC1_FormatTypeDesc_IN48(fill, p + n, maxsize - n);		/* USB Microphone Type I Format Type Descriptor (CODE == 6) 48000 */
	n += UAC1_EP_Desc_IN48(fill, p + n, maxsize - n, opts, epin, offset);	/* Endpoint Descriptor USBD_EP_AUDIO_IN In, Isochronous, 125 us */
		...
		uint_fast8_t offset)
{
	...
	// IN data flow: demodulator
	// INTERFACE_AUDIO_MIKE - audio streaming interface
	n += UAC1_InterfaceDesc(fill, p + n, maxsize - n, mikeifv, UACINALT_NONE, 0, offset, iInterfaceIN48_INRTS);	/* USB Microphone Standard AS Interface Descriptor (Alt. Set. 0) (CODE == 3) */ //zero-bandwidth interface

	// IN data flow: demodulator
	n += UAC1_InterfaceDesc(fill, p + n, maxsize - n, mikeifv, UACINALT_AUDIO48, 1, offset, iInterfaceIN48_INRTS);	/* INTERFACE_AUDIO_MIKE Interface Descriptor 2/1 Audio, 1 Endpoint, bAlternateSetting=0x01 */
	n += UAC1_AS_InterfaceDescPCM(fill, p + n, maxsize - n, terminalID);	/* USB Microphone Class-specific AS General Interface Descriptor (for output TERMINAL_UACIN48_UACINRTS) (CODE == 5) */
	n += UAC1_FormatTypeDesc_IN48(fill, p + n, maxsize - n);		/* USB Microphone Type I Format Type Descriptor (CODE == 6) 48000 */
	n += UAC1_EP_Desc_IN48(fill, p + n, maxsize - n, opts, epin, offset);	/* Endpoint Descriptor USBD_EP_AUDIO_IN In, Isochronous, 125 us */
	n += UAC1_AS_EP_Desc(fill, p + n, maxsize - n);	/* USB Microphone Class-specific Isoc. Audio Data Endpoint Descriptor (CODE == 7) OK - подтверждено документацией*/
...
#if WITHRTS96
	// IN data flow: radio RX spectrum data
	n += UAC1_InterfaceDesc(fill, p + n, maxsize - n, mikeifv, UACINALT_RTS96, 1, offset, iInterfaceIN48_INRTS);	/* INTERFACE_AUDIO_MIKE Interface Descriptor 2/1 Audio, 1 Endpoint, bAlternateSetting=0x01 */
	n += UAC1_AS_InterfaceDescPCM(fill, p + n, maxsize - n, terminalID);	/* USB Microphone Class-specific AS General Interface Descriptor (for output TERMINAL_UACIN48_UACINRTS) (CODE == 5) */
	n += UAC1_FormatTypeDesc_RTS96(fill, p + n, maxsize - n);		/* USB Microphone Type I Format Type Descriptor (CODE == 6) 48000 */
	n += UAC1_EP_Desc_RTS96(fill, p + n, maxsize - n, opts, epin, offset);	/* Endpoint Descriptor USBD_EP_AUDIO_IN In, Isochronous, 125 us */
	n += UAC1_AS_EP_Desc(fill, p + n, maxsize - n);	/* USB Microphone Class-specific Isoc. Audio Data Endpoint Descriptor (CODE == 7) OK - подтверждено документацией*/
#endif /* WITHRTS96 */

#if WITHRTS192
	// IN data flow: radio RX spectrum data
	n += UAC1_InterfaceDesc(fill, p + n, maxsize - n, mikeifv, UACINALT_RTS192, 1, offset, iInterfaceIN48_INRTS);	/* INTERFACE_AUDIO_MIKE Interface Descriptor 2/1 Audio, 1 Endpoint, bAlternateSetting=0x01 */
	n += UAC1_AS_InterfaceDescPCM(fill, p + n, maxsize - n, terminalID);	/* USB Microphone Class-specific AS General Interface Descriptor (for output TERMINAL_UACIN48_UACINRTS) (CODE == 5) */
	n += UAC1_FormatTypeDesc_RTS192(fill, p + n, maxsize - n);		/* USB Microphone Type I Format Type Descriptor (CODE == 6) 48000 */
	n += UAC1_EP_Desc_RTS192(fill, p + n, maxsize - n, opts, epin, offset);	/* Endpoint Descriptor USBD_EP_AUDIO_IN In, Isochronous, 125 us */
	n += UAC1_AS_EP_Desc(fill, p + n, maxsize - n);	/* USB Microphone Class-specific Isoc. Audio Data Endpoint Descriptor (CODE == 7) OK - подтверждено документацией*/
#endif /* WITHRTS192 */
#endif /* ! WITHUSBUACIN2 */
	return n;
	...
	uint_fast8_t offset)
{
	...
	// IN data flow: radio RX audio data
	n += UAC1_InterfaceDesc(fill, p + n, maxsize - n, mikeifv, UACINALT_AUDIO48, 1, offset, iInterfaceIN48);	/* INTERFACE_AUDIO_MIKE Interface Descriptor 2/1 Audio, 1 Endpoint, bAlternateSetting=0x01 */
	n += UAC1_AS_InterfaceDescPCM(fill, p + n, maxsize - n, terminalInID);	/* USB Microphone Class-specific AS General Interface Descriptor (for output TERMINAL_UACIN48_UACINRTS) (CODE == 5) */
	n += UAC1_FormatTypeDesc_IN48(fill, p + n, maxsize - n);		/* USB Microphone Type I Format Type Descriptor (CODE == 6) 48000 */
	n += UAC1_EP_Desc_IN48(fill, p + n, maxsize - n, opts, epin, offset);	/* Endpoint Descriptor USBD_EP_AUDIO_IN In, Isochronous, 125 us */
	n += UAC1_AS_EP_Desc(fill, p + n, maxsize - n);	/* USB Microphone Class-specific Isoc. Audio Data Endpoint Descriptor (CODE == 7) OK - ???????????? ?????????????*/
	...
	#if WITHRTS96
		// IN data flow: radio RX spectrum data
		n += UAC1_InterfaceDesc(fill, p + n, maxsize - n, mikeifv, UACINALT_RTS96, 1, offset, iInterfaceINRTS);	/* INTERFACE_AUDIO_MIKE Interface Descriptor 2/1 Audio, 1 Endpoint, bAlternateSetting=0x01 */
		n += UAC1_AS_InterfaceDescPCM(fill, p + n, maxsize - n, terminalInID);	/* USB Microphone Class-specific AS General Interface Descriptor (for output TERMINAL_UACIN48_UACINRTS) (CODE == 5) */
		n += UAC1_FormatTypeDesc_RTS96(fill, p + n, maxsize - n);		/* USB Microphone Type I Format Type Descriptor (CODE == 6) 48000 */
		n += UAC1_EP_Desc_RTS96(fill, p + n, maxsize - n, opts, epin, offset);	/* Endpoint Descriptor USBD_EP_AUDIO_IN In, Isochronous, 125 us */
		n += UAC1_AS_EP_Desc(fill, p + n, maxsize - n);	/* USB Microphone Class-specific Isoc. Audio Data Endpoint Descriptor (CODE == 7) OK - подтверждено документацией*/
	...
	#if WITHRTS192
		// IN data flow: radio RX spectrum data
		n += UAC1_InterfaceDesc(fill, p + n, maxsize - n, mikeifv, UACINALT_RTS192, 1, offset, iInterfaceINRTS);	/* INTERFACE_AUDIO_MIKE Interface Descriptor 2/1 Audio, 1 Endpoint, bAlternateSetting=0x01 */
		n += UAC1_AS_InterfaceDescPCM(fill, p + n, maxsize - n, terminalInID);	/* USB Microphone Class-specific AS General Interface Descriptor (for output TERMINAL_UACIN48_UACINRTS) (CODE == 5) */
		n += UAC1_FormatTypeDesc_RTS192(fill, p + n, maxsize - n);		/* USB Microphone Type I Format Type Descriptor (CODE == 6) 48000 */
		n += UAC1_EP_Desc_RTS192(fill, p + n, maxsize - n, opts, epin, offset);	/* Endpoint Descriptor USBD_EP_AUDIO_IN In, Isochronous, 125 us */
		n += UAC1_AS_EP_Desc(fill, p + n, maxsize - n);	/* USB Microphone Class-specific Isoc. Audio Data Endpoint Descriptor (CODE == 7) OK - подтверждено документацией*/
	...
	// OUT data flow: modulator
	// INTERFACE_AUDIO_SPK - audio streaming interface
	n += UAC1_InterfaceDesc(fill, p + n, maxsize - n, modulatorifv, UACOUTALT_NONE, 0, offset, iInterfaceOUT48);	/* INTERFACE_AUDIO_SPK - Interface 1, Alternate Setting 0 */
...
static unsigned fill_UAC2_IN48_OUT48_function(
	uint_fast8_t fill, uint8_t * p, unsigned maxsize,
	const desc_options_t * opts,
	uint_fast8_t offset	// UACOFFS_IN48_OUT48 here
	)
{
	unsigned n = 0;
...
//		    .bOSC = UACTEix(TERMINAL_ID_CLKSOURCE, offset),
//		    .bCSourceID = UACTEix(TERMINAL_ID_CLKMULTIPLIER, offset),
			.bCSourceID = UACTEix(TERMINAL_ID_CLKSOURCE, offset),	// UACOFFS_IN48_OUT48
	};
	const tpgt_t modulatorterms =
	{
			.bTerminalID = terminalOutID,
...
static unsigned fill_UAC1_function(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts)
{
	unsigned n = 0;

#if WITHUSBUACIN && WITHUSBUACOUT && WITHUSBUACINOUTRENESAS
	/* совмещённое усройство ввода/вывода (и спектр измененем параметров устройства) */
	n += fill_UAC1_IN48_OUT48_function(fill, p + n, maxsize - n, opts, UACOFFS_IN48_OUT48);
	...
	n += fill_UAC1_IN48_OUT48_function(fill, p + n, maxsize - n, opts, UACOFFS_IN48_OUT48);
		...
		n += fill_UAC1_INRTS_function(fill, p + n, maxsize - n, opts, UACOFFS_INRTS);
		...
		n += fill_UAC1_IN48_function(fill, p + n, maxsize - n, opts, UACOFFS_IN48);
	...
	#elif WITHUSBUACIN
		/* на одном устройстве различные форматы для передачи в компьютер спектра и звука */
		n += fill_UAC1_IN48_INRTS_function(fill, p + n, maxsize - n, opts, UACOFFS_IN48_INRTS);

	#endif /* WITHUSBUACIN2 */

	#if WITHUSBUACOUT

		n += fill_UAC1_OUT48_function(fill, p + n, maxsize - n, opts, UACOFFS_OUT48);
	#endif /* WITHUSBUACOUT */
...
static unsigned fill_UAC2_function(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts)
{
	unsigned n = 0;

#if WITHUSBUACIN && WITHUSBUACOUT && WITHUSBUACINOUTRENESAS
	/* совмещённое усройство ввода/вывода (и спектр измененем параметров устройства) */
	n += fill_UAC2_IN48_OUT48_function(fill, p + n, maxsize - n, opts, UACOFFS_IN48_OUT48);

#elif WITHUSBUACIN && WITHUSBUACOUT && WITHUSBUACINOUT
	/* отдельные функции для передачи в компьютер спектра и двунапаправленная звука */
	n += fill_UAC2_IN48_OUT48_function(fill, p + n, maxsize - n, opts, UACOFFS_IN48_OUT48);
	#if (WITHRTS96 || WITHRTS192) && WITHUSBUACIN2
		n += fill_UAC2_INRTS_function(fill, p + n, maxsize - n, opts, UACOFFS_INRTS);
	#endif /*(WITHRTS96 || WITHRTS192) && WITHUSBUACIN2 */

#else /* WITHUSBUACIN && WITHUSBUACOUT && WITHUSBUACINOUT */
	#if WITHUSBUACIN2
		/* отдельные функции для передачи в компьютер спектра и звука */
		n += fill_UAC2_IN48_function(fill, p + n, maxsize - n, opts, UACOFFS_IN48);
		#if WITHRTS96 || WITHRTS192
			n += fill_UAC2_INRTS_function(fill, p + n, maxsize - n, opts, UACOFFS_INRTS);
		#else /* WITHRTS96 || WITHRTS192 */
			#error WITHRTS96 or WITHRTS192 required for WITHUSBUACIN2
	...
	#elif WITHUSBUACIN
		/* на одном устройстве различные форматы для передачи в компьютер спектра и звука */
		n += fill_UAC2_IN48_INRTS_function(fill, p + n, maxsize - n, opts, UACOFFS_IN48_INRTS);

	#endif /* WITHUSBUACIN2 */

	#if WITHUSBUACOUT

		n += fill_UAC2_OUT48_function(fill, p + n, maxsize - n, opts, UACOFFS_OUT48);
	#endif /* WITHUSBUACOUT */
...
static unsigned CDCACM_Control_EP(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, const desc_options_t * opts, uint_fast8_t bEndpointAddress)
{
	...
	if (fill != 0 && buff != NULL)
	{
		const uint_fast16_t wMaxPacketSize = encodeMaxPacketSize(opts->hsdesc, VIRTUAL_COM_PORT_NOTIFY_SIZE);
		// Вызов для заполнения, а не только для проверки занимаемого места в буфере
		* buff ++ = length;							/* bLength */
...
static unsigned CDCACM_Data_EP_OUT(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, const desc_options_t * opts, uint_fast8_t bEndpointAddress)
{
	...
	if (fill != 0 && buff != NULL)
	{
		const uint_fast16_t wMaxPacketSize = encodeMaxPacketSize(opts->hsdesc, VIRTUAL_COM_PORT_OUT_DATA_SIZE);
		// Вызов для заполнения, а не только для проверки занимаемого места в буфере
		* buff ++ = length;							/* bLength */
		* buff ++ = USB_ENDPOINT_DESCRIPTOR_TYPE;   /* bDescriptorType: Endpoint */
...
static unsigned CDCACM_Data_EP_IN(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, const desc_options_t * opts, uint_fast8_t bEndpointAddress)
{
	...
	if (fill != 0 && buff != NULL)
	{
		const uint_fast16_t wMaxPacketSize = encodeMaxPacketSize(opts->hsdesc, VIRTUAL_COM_PORT_IN_DATA_SIZE);
		// Вызов для заполнения, а не только для проверки занимаемого места в буфере
		* buff ++ = length;							/* bLength */
		* buff ++ = USB_ENDPOINT_DESCRIPTOR_TYPE;	/* bDescriptorType: Endpoint */
...
static unsigned fill_CDCACM_function_a(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts, uint_fast8_t offset)
{
	const uint8_t highspeed = opts->hsdesc;
	unsigned n = 0;
...
#if ! WITHUSBCDCACM_NOINT
	const uint8_t intnep = USBD_CDCACM_NOTIFY_EP(USBD_EP_CDCACM_NOTIFY, offset);;
#endif /* ! WITHUSBCDCACM_NOINT */
...
// Group of CDC
static unsigned fill_CDCACM_function(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts)
{
	unsigned offset;
	unsigned n = 0;

	for (offset = 0; offset < WITHUSBCDCACM_N; ++ offset)
		n += fill_CDCACM_function_a(fill, p + n, maxsize - n, opts, offset);
	return n;
}
...
static unsigned fill_CDCEEM_function(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts)
{
	unsigned n = 0;
...
static unsigned CDCECM_fill_35(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, const desc_options_t * opts, uint_fast8_t bEndpointAddress)
{
	...
	if (fill != 0 && buff != NULL)
	{
		const uint_fast16_t wMaxPacketSize = encodeMaxPacketSize(opts->hsdesc, USBD_CDCECM_NOTIFY_SIZE);
		// Вызов для заполнения, а не только для проверки занимаемого места в буфере
		* buff ++ = length;							/* bLength */
		* buff ++ = USB_ENDPOINT_DESCRIPTOR_TYPE; 	/* bDescriptorType: Endpoint */
		* buff ++ = bEndpointAddress;			/* bEndpointAddress: (IN) */
...
static unsigned fill_CDCECM_function(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts)
{
	...
	n += CDCECM_fill_35(fill, p + n, maxsize - n, opts, USB_ENDPOINT_IN(USBD_EP_CDCECM_NOTIFY));	/* Endpoint Descriptor 86 6 In, Interrupt */
...
static unsigned CDCNCM_fill_35(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, const desc_options_t * opts, uint_fast8_t bEndpointAddress)
{
	...
	if (fill != 0 && buff != NULL)
	{
		const uint_fast16_t wMaxPacketSize = encodeMaxPacketSize(opts->hsdesc, USBD_CDCNCM_NOTIFY_SIZE);
		// Вызов для заполнения, а не только для проверки занимаемого места в буфере
		* buff ++ = length;							/* bLength */
		* buff ++ = USB_ENDPOINT_DESCRIPTOR_TYPE; 	/* bDescriptorType: Endpoint */
		* buff ++ = bEndpointAddress;			/* bEndpointAddress: (IN) */
...
static unsigned fill_CDCNCM_function(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts)
{
	...
	n += CDCNCM_fill_35(fill, p + n, maxsize - n, opts, USB_ENDPOINT_IN(USBD_EP_CDCNCM_NOTIFY));	/* Endpoint Descriptor 86 6 In, Interrupt */
...
static unsigned RNDIS_fill_35(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, const desc_options_t * opts, uint_fast8_t bEndpointAddress)
{
	...
	if (fill != 0 && buff != NULL)
	{
		const uint_fast16_t wMaxPacketSize = encodeMaxPacketSize(opts->hsdesc, USBD_RNDIS_NOTIFY_SIZE);
		// Вызов для заполнения, а не только для проверки занимаемого места в буфере
		* buff ++ = length;							/* bLength */
		* buff ++ = USB_ENDPOINT_DESCRIPTOR_TYPE; 	/* bDescriptorType: Endpoint */
		* buff ++ = bEndpointAddress;			/* bEndpointAddress: (IN) */
		...
		* buff ++ = LO_BYTE(wMaxPacketSize);        /* wMaxPacketSize */
		* buff ++ = HI_BYTE(wMaxPacketSize); 
		* buff ++ = opts->hsdesc ? HSINTERVAL_1MS : FSINTERVAL_1MS;   		/* bInterval: 1 mS */
	}
	return length;
}
...
static unsigned RNDIS_fill_38(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, const desc_options_t * opts, uint_fast8_t bEndpointAddress)
{
	...
	if (fill != 0 && buff != NULL)
	{
		const uint_fast16_t wMaxPacketSize = encodeMaxPacketSize(opts->hsdesc, USBD_RNDIS_IN_BUFSIZE);
		// Вызов для заполнения, а не только для проверки занимаемого места в буфере
		* buff ++ = length;							/* bLength */
		* buff ++ = USB_ENDPOINT_DESCRIPTOR_TYPE;	/* bDescriptorType: Endpoint */
		* buff ++ = bEndpointAddress;				/* bEndpointAddress: (IN2) */
...
static unsigned RNDIS_fill_37(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, const desc_options_t * opts, uint_fast8_t bEndpointAddress)
{
	...
	if (fill != 0 && buff != NULL)
	{
		const uint_fast16_t wMaxPacketSize = encodeMaxPacketSize(opts->hsdesc, USBD_RNDIS_OUT_BUFSIZE);
		// Вызов для заполнения, а не только для проверки занимаемого места в буфере
		* buff ++ = length;							/* bLength */
		* buff ++ = USB_ENDPOINT_DESCRIPTOR_TYPE;   /* bDescriptorType: Endpoint */
		* buff ++ = bEndpointAddress;				/* bEndpointAddress: (OUT2) */
...
/* CDC Ethernet Control Model */
static unsigned fill_RNDIS_function(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts)
{
	const int highspeed = opts->hsdesc;
	unsigned n = 0;
	// Interface Association Descriptor RF Controller
	...
	n += RNDIS_fill_35(fill, p + n, maxsize - n, opts, USB_ENDPOINT_IN(USBD_EP_RNDIS_NOTIFY));	/* Endpoint Descriptor 86 6 In, Interrupt */
...
static unsigned HID_InterfaceDescXXXX(uint_fast8_t fill, uint8_t * buff, unsigned maxsize)
{
	...
	if (fill != 0 && buff != NULL)
	{
		...
		* buff ++ = 0x00;								/* bAlternateSetting */
#if defined (HIDKEYBOARD_NOTIFY_DATA_SIZE)
...
#elif defined (HIDMOUSE_NOTIFY_DATA_SIZE)
		* buff ++ = 0x01;								/* bNumEndpoints */
		* buff ++ = USB_DEVICE_CLASS_HUMAN_INTERFACE;   /* bInterfaceClass */
...
#if defined (HIDMOUSE_NOTIFY_DATA_SIZE)
...
static unsigned fill_HID_Mouse_IntEP(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, int highspeed, uint_fast8_t bEndpointAddress)
{
	...
	if (fill != 0 && buff != NULL)
	{
		const uint_fast16_t wMaxPacketSize = encodeMaxPacketSize(opts->hsdesc, HIDMOUSE_NOTIFY_DATA_SIZE);
		// Вызов для заполнения, а не только для проверки занимаемого места в буфере
		* buff ++ = length;							/* bLength */
		* buff ++ = USB_ENDPOINT_DESCRIPTOR_TYPE; 	/* bDescriptorType: Endpoint */
		* buff ++ = bEndpointAddress;			/* bEndpointAddress: (IN) */
...
#endif /* defined (HIDMOUSE_NOTIFY_DATA_SIZE) */
...
static unsigned fill_HID_Keyboard_IntEP(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, int highspeed, uint_fast8_t bEndpointAddress)
{
	...
	if (fill != 0 && buff != NULL)
	{
		const uint_fast16_t wMaxPacketSize = encodeMaxPacketSize(opts->hsdesc, HIDKEYBOARD_NOTIFY_DATA_SIZE);
		// Вызов для заполнения, а не только для проверки занимаемого места в буфере
		* buff ++ = length;							/* bLength */
		* buff ++ = USB_ENDPOINT_DESCRIPTOR_TYPE; 	/* bDescriptorType: Endpoint */
		* buff ++ = bEndpointAddress;			/* bEndpointAddress: (IN) */
...
#endif /* defined (HIDKEYBOARD_NOTIFY_DATA_SIZE) */
...
static unsigned fill_HID_XXXX_function(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts)
{
...
#if defined (HIDMOUSE_NOTIFY_DATA_SIZE)
	n += fill_HID_Mouse_IntEP(fill, p + n, maxsize - n, highspeed, USB_ENDPOINT_IN(USBD_EP_HIDMOUSE_NOTIFY));
#elif defined (HIDKEYBOARD_NOTIFY_DATA_SIZE)
	n += fill_HID_Keyboard_IntEP(fill, p + n, maxsize - n, highspeed, USB_ENDPOINT_IN(USBD_EP_HIDKEYBOARD_NOTIFY));
#else
#endif
	return n;
}
...
static unsigned fill_DFU_function(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts)
{
	uint_fast8_t ialt = 0;
	unsigned n = 0;
	//
...
static unsigned fill_MTP_XXXX_function(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts)
{
	unsigned n = 0;
	//
	...
	n += MTP_EndpointInt(fill, p + n, maxsize - n, highspeed, USBD_EP_MTP_NOTIFY);
	//
	return n;
}
...
static unsigned MSC_EndpointIn(uint_fast8_t fill, uint8_t * buff, unsigned maxsize, const desc_options_t * opts, uint_fast8_t bEndpointAddress)
{
	...
	if (fill != 0 && buff != NULL)
	{
		const uint_fast16_t wMaxPacketSize = encodeMaxPacketSize(opts->hsdesc, opts->hsdesc ? MSC_DATA_MAX_PACKET_SIZE_HS : MSC_DATA_MAX_PACKET_SIZE_FS); // was: 0x300
		// Вызов для заполнения, а не только для проверки занимаемого места в буфере
		* buff ++ = length;							/* bLength */
		* buff ++ = USB_ENDPOINT_DESCRIPTOR_TYPE;	// bDescriptorType
		* buff ++ = bEndpointAddress;                    // bEndpointAddress
...
static unsigned fill_MSC_XXXX_function(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts)
{
	unsigned n = 0;
	//
...
static unsigned fill_Configuration_compound(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts)
{
	unsigned n = 0;

#if WITHUSBDFU
	n += fill_DFU_function(fill, p + n, maxsize - n, opts);
#endif /* WITHUSBDFU */

#if WITHUSBCDCACM
	/* создаем одно или несколько (WITHUSBCDCACM_N) устройств */
	n += fill_CDCACM_function(fill, p + n, maxsize - n, opts);
#endif /* WITHUSBCDCACM */

#if WITHUSBUAC
	#if WITHUAC2
		n += fill_UAC2_function(fill, p + n, maxsize - n, opts);
	#else /* WITHUAC2 */
		n += fill_UAC1_function(fill, p + n, maxsize - n, opts);
	#endif /* WITHUAC2 */
#endif /* WITHUSBUAC */

#if WITHUSBRNDIS
	n += fill_RNDIS_function(fill, p + n, maxsize - n, opts);
#endif /* WITHUSBRNDIS */

#if WITHUSBCDCEEM
	n += fill_CDCEEM_function(fill, p + n, maxsize - n, opts);
#endif /* WITHUSBCDCEEM */
...
#if WITHUSBDMTP
	n += fill_MTP_XXXX_function(fill, p + n, maxsize - n, opts);
#endif /* WITHUSBDMTP */

#if WITHUSBDMSC
	n += fill_MSC_XXXX_function(fill, p + n, maxsize - n, opts);
#endif /* WITHUSBDMSC */

	return n;
}

typedef unsigned (* fill_func_t)(uint_fast8_t fill, uint8_t * p, unsigned maxsize, const desc_options_t * opts);
...
static unsigned fill_Configuration_descriptor(
		uint8_t * buff, unsigned maxsize, const desc_options_t * opts,
...
{
	unsigned length = 9;
	unsigned totalsize = length + fill_main_group(0, buff, maxsize - length, opts);
	ASSERT(maxsize >= length);
	if (maxsize < length)
		return 0;
	if (buff != NULL)
	{
		...
		* buff ++ = USB_CONFIG_POWER_MA(250);/* bMaxPower = 250 mA. Сделано как попытка улучшить работу через активные USB изоляторы для обеспечения их питания. */
		...
		fill_main_group(1, buff, maxsize - length, opts);
	}
	return totalsize;
}

// Device Descriptor
static unsigned fill_Device_descriptor(uint8_t * buff, unsigned maxsize, uint_fast8_t bNumConfigurations, const desc_options_t * opts)
{
	const unsigned length = 18;
	ASSERT(maxsize >= length);
	if (maxsize < length)
		return 0;
	const uint_fast16_t iVendorId = usbd_get_vendorId(opts);
	const uint_fast16_t iProductId = usbd_get_productId(opts);
	if (buff != NULL)
	{
		// Вызов для заполнения, а не только для проверки занимаемого места в буфере
		* buff ++ = length;					    /*  0:bLength */
...
void usbd_descriptors_initialize(uint_fast8_t HSdescv, uint_fast8_t ft8cnv)
{
	unsigned score = 0;
	unsigned offset;
...

	{
		...
		partlen = fill_Device_descriptor(alldescbuffer + score, ARRAY_SIZE(alldescbuffer) - score, bNumConfigurations, & opts0);
...

	{
		...
		for (index = 0; index < bNumConfigurations; ++ index)
		{
			...
			score += fill_align4(alldescbuffer + score, ARRAY_SIZE(alldescbuffer) - score);
			partlen = fill_Configuration_descriptor(alldescbuffer + score, ARRAY_SIZE(alldescbuffer) - score, & opts0, funcs [index].confvalue, funcs [index].count, funcs [index].fp);
...
//	}
	...
	if ((USB_FUNCTION_BCD_USB >= 0x0201) || (HSdescv != 0))
	{
		unsigned partlen;
		// Device Qualifier
		score += fill_align4(alldescbuffer + score, ARRAY_SIZE(alldescbuffer) - score);
	...
	{
//		PRINTF("UACTEix(TERMINAL_ID_USBSTREAM, UACOFFS_OUT48)=%u\n", UACTEix(TERMINAL_ID_USBSTREAM, UACOFFS_OUT48));
//		PRINTF("UACTEix(TERMINAL_ID_USBSTREAM, UACOFFS_IN48)=%u\n", UACTEix(TERMINAL_ID_USBSTREAM, UACOFFS_IN48));
//		PRINTF("UACTEix(TERMINAL_ID_USBSTREAM, UACOFFS_IN48_OUT48)=%u\n", UACTEix(TERMINAL_ID_USBSTREAM, UACOFFS_IN48_OUT48));
//		PRINTF("UACTEix(TERMINAL_ID_USBSTREAM, UACOFFS_IN48_INRTS)=%u\n", UACTEix(TERMINAL_ID_USBSTREAM, UACOFFS_IN48_INRTS));
//		PRINTF("UACTEix(TERMINAL_ID_USBSTREAM, UACOFFS_INRTS)=%u\n", UACTEix(TERMINAL_ID_USBSTREAM, UACOFFS_INRTS));
//
//		PRINTF("UACTEix(TERMINAL_ID_CLKSOURCE, UACOFFS_OUT48)=%u\n", UACTEix(TERMINAL_ID_CLKSOURCE, UACOFFS_OUT48));
//		PRINTF("UACTEix(TERMINAL_ID_CLKSOURCE, UACOFFS_IN48)=%u\n", UACTEix(TERMINAL_ID_CLKSOURCE, UACOFFS_IN48));
//		PRINTF("UACTEix(TERMINAL_ID_CLKSOURCE, UACOFFS_IN48_OUT48)=%u\n", UACTEix(TERMINAL_ID_CLKSOURCE, UACOFFS_IN48_OUT48));
//		PRINTF("UACTEix(TERMINAL_ID_CLKSOURCE, UACOFFS_IN48_INRTS)=%u\n", UACTEix(TERMINAL_ID_CLKSOURCE, UACOFFS_IN48_INRTS));
//		PRINTF("UACTEix(TERMINAL_ID_CLKSOURCE, UACOFFS_INRTS)=%u\n", UACTEix(TERMINAL_ID_CLKSOURCE, UACOFFS_INRTS));
//
//		PRINTF("UACTEix(TERMINAL_ID_FU2a_IN, UACOFFS_OUT48)=%u\n", UACTEix(TERMINAL_ID_FU2a_IN, UACOFFS_OUT48));
//		PRINTF("UACTEix(TERMINAL_ID_FU2a_IN, UACOFFS_IN48)=%u\n", UACTEix(TERMINAL_ID_FU2a_IN, UACOFFS_IN48));
//		PRINTF("UACTEix(TERMINAL_ID_FU2a_IN, UACOFFS_IN48_OUT48)=%u\n", UACTEix(TERMINAL_ID_FU2a_IN, UACOFFS_IN48_OUT48));
//		PRINTF("UACTEix(TERMINAL_ID_FU2a_IN, UACOFFS_IN48_INRTS)=%u\n", UACTEix(TERMINAL_ID_FU2a_IN, UACOFFS_IN48_INRTS));
//		PRINTF("UACTEix(TERMINAL_ID_FU2a_IN, UACOFFS_INRTS)=%u\n", UACTEix(TERMINAL_ID_FU2a_IN, UACOFFS_INRTS));
//
//		PRINTF("UACTEix(TERMINAL_ID_FU2b_IN, UACOFFS_OUT48)=%u\n", UACTEix(TERMINAL_ID_FU2b_IN, UACOFFS_OUT48));
//		PRINTF("UACTEix(TERMINAL_ID_FU2b_IN, UACOFFS_IN48)=%u\n", UACTEix(TERMINAL_ID_FU2b_IN, UACOFFS_IN48));
//		PRINTF("UACTEix(TERMINAL_ID_FU2b_IN, UACOFFS_IN48_OUT48)=%u\n", UACTEix(TERMINAL_ID_FU2b_IN, UACOFFS_IN48_OUT48));
//		PRINTF("UACTEix(TERMINAL_ID_FU2b_IN, UACOFFS_IN48_INRTS)=%u\n", UACTEix(TERMINAL_ID_FU2b_IN, UACOFFS_IN48_INRTS));
//		PRINTF("UACTEix(TERMINAL_ID_FU2b_IN, UACOFFS_INRTS)=%u\n", UACTEix(TERMINAL_ID_FU2b_IN, UACOFFS_INRTS));
//
//		PRINTF("UACTEix(TERMINAL_ID_FU2a_OUT, UACOFFS_OUT48)=%u\n", UACTEix(TERMINAL_ID_FU2a_OUT, UACOFFS_OUT48));
//		PRINTF("UACTEix(TERMINAL_ID_FU2a_OUT, UACOFFS_IN48)=%u\n", UACTEix(TERMINAL_ID_FU2a_OUT, UACOFFS_IN48));
//		PRINTF("UACTEix(TERMINAL_ID_FU2a_OUT, UACOFFS_IN48_OUT48)=%u\n", UACTEix(TERMINAL_ID_FU2a_OUT, UACOFFS_IN48_OUT48));
//		PRINTF("UACTEix(TERMINAL_ID_FU2a_OUT, UACOFFS_IN48_INRTS)=%u\n", UACTEix(TERMINAL_ID_FU2a_OUT, UACOFFS_IN48_INRTS));
//		PRINTF("UACTEix(TERMINAL_ID_FU2a_OUT, UACOFFS_INRTS)=%u\n", UACTEix(TERMINAL_ID_FU2a_OUT, UACOFFS_INRTS));
//
//		PRINTF("UACTEix(TERMINAL_ID_FU2b_OUT, UACOFFS_OUT48)=%u\n", UACTEix(TERMINAL_ID_FU2b_OUT, UACOFFS_OUT48));
//		PRINTF("UACTEix(TERMINAL_ID_FU2b_OUT, UACOFFS_IN48)=%u\n", UACTEix(TERMINAL_ID_FU2b_OUT, UACOFFS_IN48));
//		PRINTF("UACTEix(TERMINAL_ID_FU2b_OUT, UACOFFS_IN48_OUT48)=%u\n", UACTEix(TERMINAL_ID_FU2b_OUT, UACOFFS_IN48_OUT48));
//		PRINTF("UACTEix(TERMINAL_ID_FU2b_OUT, UACOFFS_IN48_INRTS)=%u\n", UACTEix(TERMINAL_ID_FU2b_OUT, UACOFFS_IN48_INRTS));
//		PRINTF("UACTEix(TERMINAL_ID_FU2b_OUT, UACOFFS_INRTS)=%u\n", UACTEix(TERMINAL_ID_FU2b_OUT, UACOFFS_INRTS));
	}
}
