...
//		;
	...
	GIC_SendSGI(SGI8_IRQn, UINT32_C(1) << targetcore, 0x00);	// CPU1, filer=0 Forward the interrupt to the CPU interfaces specified in the CPUTargetList field
}
...
unsigned
ttb_mempage_accessbits(const mmulayout_t * layout, const getmmudesc_t * arch, uint8_t * b, uint_fast64_t phyaddr, int ro, int xn)
{
	...
	if (phyaddr == 0x00000000)
		return arch->mnoaccess(b, phyaddr);		// NULL pointers access trap

	if (phyaddr >= 0x18000000 && phyaddr < 0x20000000)			// FIXME: QSPI memory mapped should be R/O, but...
		return arch->mcached(b, phyaddr, ro || 0, xn);
...
//		return arch->mcached(b, phyaddr, ro, xn);
	...
	if (phyaddr < 0x01000000)
		return arch->mcached(b, phyaddr, ro, xn);	// SRAM A1, SRAM A2, SRAM C

	if (phyaddr >= 0xC0000000)
		return arch->mcached(b, phyaddr, ro, xn);	// N-BROM, S-BROM
