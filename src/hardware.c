...
{
	unsigned (* const poke)(uint8_t * b, uint_fast64_t v) = layout->poke;
	...
	if (phyaddr >= __ramnc_base && phyaddr < __ramnc_top)			// non-cached DRAM
		return arch->mncached(poke, b, phyaddr, ro, 1 || xn);
#endif
	...
	if (phyaddr == 0x00000000)
		return arch->mnoaccess(poke, b, phyaddr);		// NULL pointers access trap

	if (phyaddr >= 0x18000000 && phyaddr < 0x20000000)			// FIXME: QSPI memory mapped should be R/O, but...
		return arch->mcached(poke, b, phyaddr, ro || 0, xn);
	...
	return arch->mdevice(poke, b, phyaddr);
	...
	return arch->mdevice(poke, b, phyaddr);
	return arch->mnoaccess(poke, b, phyaddr);
...
//		return arch->mcached(poke, b, phyaddr, ro, xn);
	...
	if (phyaddr >= 0xC0000000)
		return arch->mcached(poke, b, phyaddr, ro, xn);	// N-BROM, S-BROM
