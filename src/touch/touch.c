...
void board_tsc_calibration(void)
{
	...
	if (0)
	{
		...
		for (tg = 0; tg < TSCCALIBPOINTS; ++ tg)
		{
			...
			unifont_text(& dbv, xstep * 2, ystep * 5, & unifont_small, "CALIBRATE", TEZXTSZIE_AUTO, COLOR_WHITEALL);
			colmain_nextfb();
			// wait answer
			unsigned as;
			for (as = 0; as < 5000; /*++ as */)
			...
			// стереть фон
			colpip_fillrect(& dbv, 0, 0, DIM_X, DIM_Y, COLOR_BLACK);
			unifont_text(& dbv, xstep * 2, ystep * 5, & unifont_small, "CALIBRATE DONE", TEZXTSZIE_AUTO, COLOR_WHITEALL);
			colmain_nextfb();
