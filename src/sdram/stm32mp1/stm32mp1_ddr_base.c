...
int stm32mp1_ddr_clk_enable_local(struct ddr_info *priv, uint32_t mem_speed)
{
	unsigned long ddrphy_clk, ddr_clk, mem_speed_hz;

	ddr_enable_clock_local();
	       ...
	       struct stm32mp1_ddr_config_local *config)
{
	...
	/* 1.2. start CLOCK */
	if (stm32mp1_ddr_clk_enable_local(priv, config->info.speed) != 0) {
		panic();
	}
	...
	VERBOSE("stm32mp1_ddr_init_priv: DX0DQSTR=%08lX, DX1DQSTR=%08lX, DX2DQSTR=%08lX, DX3DQSTR=%08lX\n",
...
#endif /* WITHSDRAMHW && CPUSTYLE_STM32MP1 */
