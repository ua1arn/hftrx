...
uint_fast16_t
cpfont_totalci(const unifont_t * font)
{
	const uint8_t * const blob = (const uint8_t * const) font->fontraster;
	const unsigned first = USBD_peek_u16(blob + 2);
	const unsigned last = USBD_peek_u16(blob + 4);
...
const void * cpfont_getcharraster(const struct unifont_tag * font, uint_fast16_t ci)
{
	const uint8_t * const blob = (const uint8_t * const) font->fontraster;
	enum { PARAMSIZE = 6 };
	...
	const uint_fast16_t rastersstart = indexstart + totalci + totalci * PARAMSIZE;
	return NULL;
}

uint_fast8_t cpfont_width(const struct unifont_tag * font, uint_fast16_t ci)	// ширина в пиксеях данного символа на экране
{
	const uint8_t * const blob = (const uint8_t * const) font->fontraster;
	enum { PARAMSIZE = 6 };
	const uint_fast16_t totalci = font->totalci(font);
	...
	const uint_fast16_t rastersstart = indexstart + totalci + totalci * PARAMSIZE;
	PRINTF("cpfont_width:\n");
	printhex(0, blob + paramstart, PARAMSIZE);
	return 0;
}

uint_fast8_t cpfont_height(const struct unifont_tag * font)	// высота в пикселях (все символы шрифта одной высоты)
{
	const uint8_t * const blob = (const uint8_t * const) font->fontraster;
	return USBD_peek_u16(blob + 8);
}

uint_fast16_t cpfont_draw(const gxdrawb_t * db, uint_fast16_t xpix, uint_fast16_t ypix, const struct unifont_tag * font, uint_fast16_t ci, COLORPIP_T fg)
{
	const uint8_t * const blob = (const uint8_t * const) font->fontraster;
...
const unifont_t unifont_Tahoma_Regular_88x77 =
{
	...
	.totalci = cpfont_totalci,
	.getcharrasterci = cpfont_getcharraster,
	.font_drawwidthci = cpfont_width,
	.font_drawheight = cpfont_height,
	.font_drawci = cpfont_draw,
	//
