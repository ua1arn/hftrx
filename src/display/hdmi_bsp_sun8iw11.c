...
unsigned int hdmi_readl(unsigned int addr)
{
  return *(_DWORD *)(addr + hdmi_base_addr);
}
...
int bsp_hdmi_video(struct video_para *video)
{
  ...
  vic = video->vic;
  vid = get_vid(video->vic);
  glb_video.vic = vic;
  v4 = vid;
  if ( vic <= 0x15 && ((1 << vic) & 0x220044) != 0 )
    video->csc = color_space::BT601;
  else
    video->csc = color_space::BT709;
  if ( !rcal_flag )
    hdmi_phy_init();
  v5 = get_vid(video->vic);
  v6 = hdmi_readl(0x10020u);
