...
#define TCON_TV0_local ((tcon_tv0*)TCON_TV0_BASE)

void VIDEO1_PLL2(void)
{
 uint32_t v=CCU->PLL_VIDEO1_CTRL_REG;
...
void TCONTV_Init2(uint32_t mode)
{
 if(mode==DISP_TV_MOD_NTSC)
 {
  TCON_TV0_local->CTL_REG = 0x80000000 | ((525 - 480 ) << 4);   //VT-V
  TCON_TV0_local->BASIC_REG[0] = ((720 - 1) << 16) | (480 - 1); //H,V
  TCON_TV0_local->BASIC_REG[1] = ((720 - 1) << 16) | (480 - 1);
  TCON_TV0_local->BASIC_REG[2] = ((720 - 1) << 16) | (480 - 1);
  TCON_TV0_local->BASIC_REG[3] = ((858 - 1) << 16) | ( 60 - 1); //HT, HBP
  TCON_TV0_local->BASIC_REG[4] = ((525 * 2) << 16) | ( 30 - 1); //VT*2, VBP
  TCON_TV0_local->BASIC_REG[5] = (62 << 16) | 6;                //HS, VS
 }
 else //PAL
 {
  TCON_TV0_local->CTL_REG = 0x80000000 | ((625 - 576 ) << 4);   //VT-V
  TCON_TV0_local->BASIC_REG[0] = ((720 - 1) << 16) | (576 - 1); //H,V
  TCON_TV0_local->BASIC_REG[1] = ((720 - 1) << 16) | (576 - 1);
  TCON_TV0_local->BASIC_REG[2] = ((720 - 1) << 16) | (576 - 1);
  TCON_TV0_local->BASIC_REG[3] = ((864 - 1) << 16) | ( 68 - 1); //HT, HBP
  TCON_TV0_local->BASIC_REG[4] = ((625 * 2) << 16) | ( 39 - 1); //VT*2, VBP
  TCON_TV0_local->BASIC_REG[5] = (64 << 16) | 5;                //HS, VS
 }

 TCON_TV0_local->IO_POL_REG = 0;
 TCON_TV0_local->IO_TRI_REG = 0x0FFFFFFF;

 TCON_TV0_local->SRC_CTL_REG=0;             //0 - DE, 1..7 - test
 TCON_TV0_local->GINT0_REG=(1<<30);         //enable Vblank int
 TCON_TV0_local->GCTL_REG=(1UL<<31)|(1<<1); //enable TCONTV
}

//void TV_VSync2(void)
//{
// TCON_TV0_local->GINT0_REG&=~(1<<14);
// while(!(TCON_TV0_local->GINT0_REG&(1<<14)));
//}

void TVE_Init2(uint32_t mode)
{
// tve2_low_set_top_reg_base((void __iomem*)TVE_TOP_BASE);
// tve2_low_set_reg_base(0,(void __iomem*)TV_Encoder_BASE);
 ...
 tve2_low_dac_autocheck_disable(0);
// tve2_low_dac_autocheck_enable(0);

 tve2_low_set_tv_mode(0,mode,0);

 tve2_low_dac_enable(0);

 tve2_low_open(0);

 tve2_low_enhance(0,0); //0,1,2
...
// Undocumented registers ---------------------------------------------------------------
 ...
 (*(volatile uint32_t*)(DISPLAY_TOP_BASE+0x20)) |= (1<<20); //enable clk for TCON_TV0_local
...
extern void sun8i_tv2_vi_scaler_setup(uint32_t src_w,uint32_t src_h,uint32_t dst_w,uint32_t dst_h,uint32_t hscale,uint32_t vscale,uint32_t hphase,uint32_t vphase);
extern void sun8i_tv2_vi_scaler_enable(uint8_t enable);
...
static void t113_tvout2_de_enable(struct fb_t113_rgb_pdata_t * pdat)
{
	struct de_glb_t * glb = (struct de_glb_t *)(pdat->virt_de + T113_DE_MUX_GLB);
	write32((uintptr_t)&glb->dbuff, 1);
}

static void t113_tvout2_de_set_address(struct fb_t113_rgb_pdata_t * pdat, void * vram, uint8_t plane)
{
 if(plane)
 {
...
static void t113_tvout2_de_set_mode(struct fb_t113_rgb_pdata_t * pdat)
{
...
//https://elixir.bootlin.com/linux/latest/source/drivers/gpu/drm/sun4i/sun8i_tv2_vi_scaler.c
...
// mgs
 sun8i_tv2_vi_scaler_setup(src_w,src_h,dst_w,dst_h,hscale,vscale,0,0);
 sun8i_tv2_vi_scaler_enable(1);
...
//CH1 UI -----------------------------------------------------------------------------
	...
	write32((uintptr_t)&ui->cfg[0].attr,(1<<0)|(DE2_FORMAT<<8)|(0xff<<24)|(1<<16));           //верхний слой с пиксельной альфой
...
static void t113_tconlcd2_set_timing(struct fb_t113_rgb_pdata_t * pdat)
{
	...
	write32((uintptr_t)&tcon->dclk, (0xf << 28) | ((val /* /2 */ ) << 0));                     //делили на 2, дисплей шпарил на ускоренной в 2 раза развёртке
...
//GPIO инициализированы в PIO.c
	...
	t113_tvout2_de_set_mode(pdat);
	t113_tvout2_de_enable(pdat);
}
...
void LCD_SwitchAddress2(uint32_t address,uint8_t plane)
{
 t113_tvout2_de_set_address(&pdat,(void*)address,plane);
 t113_tvout2_de_enable(&pdat);
}
