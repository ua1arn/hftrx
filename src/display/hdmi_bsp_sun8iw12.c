...
//#include "hdmi_bsp.h"
//#include "hdmi_core.h"
static uintptr_t hdmi_base_addr = HDMI_TX0_BASE;
//static struct video_para glb_video;
static unsigned int hdmi_version;

//struct video_para;
static int hdmi_phy_set(unsigned int vic);
...
{{17, 11, 0, 0, 2, 5, 2, 0, 5, 0, 0, 208, 144, 12, 64, 64, 49, 0, 0} },	// 720x576p @50
{{19, 4, 0, 96, 5, 5, 2, 2, 5, 1, 0, 0, 188, 184, 40, 208, 30, 1, 1} },
{{4, 4, 0, 96, 5, 5, 2, 1, 5, 0, 0, 0, 114, 110, 40, 208, 30, 1, 1} },
{{20, 4, 0, 97, 7, 5, 4, 2, 2, 2, 0, 128, 208, 16, 44, 56, 22, 1, 1} },	// 1920x1080i @50
{{5, 4, 0, 97, 7, 5, 4, 1, 2, 0, 0, 128, 24, 88, 44, 56, 22, 1, 1} },
{{31, 2, 0, 96, 7, 5, 4, 2, 4, 2, 0, 128, 208, 16, 44, 56, 45, 1, 1} },	// 1920x1080p @50
...
static void hdmi_writel(unsigned addr, uint32_t data)
{
	asm volatile("dsb st");
	*((volatile uint32_t *)(hdmi_base_addr + addr)) = data;
}

void bsp_hdmi_write(unsigned addr, unsigned char data)
{
...
static void hdmi_udelay(unsigned long us)
{
	...
	local_delay_us(us);
			...
			/*  */
static void hdmi_phy_init(unsigned int vic)
{
...
#if 1
int bsp_hdcp_enable(int enable, unsigned int vic)
{
	...
	if (enable) {
		...
		hdmi_write(0x00C0, 1 /*video->is_hdmi */ ? (hdmi_read(0x00C0) | 0x01) : (hdmi_read(0x00C0) & 0xfe));/*hdmi mode*/
...
int bsp_hdmi_video(unsigned int vic)
{
	...
	if (1 /*video->is_hdmi */) {
		...
		hdmi_write(0x10012, 0x41);
		hdmi_write(0x10013, 0x57);
		hdmi_write(0x4045, 0 /*video->is_yuv */ ? 0x02 : 0x00);
		...
		hdmi_write(0xC045, 0 /*video->is_yuv  */? 0x00 : 0x08);
...
static void _MemoryAccessRequest(uint8_t en)
{
	hdmi_write(0x10010, 0x45);
	hdmi_write(0x10011, 0x45);
	hdmi_write(0x10012, 0x52);
...
static uint8_t _MemoryAccessGranted(void)
{
	hdmi_write(0x10010, 0x45);
	hdmi_write(0x10011, 0x45);
	hdmi_write(0x10012, 0x52);
	hdmi_write(0x10013, 0x54);
	return (uint8_t)((hdmi_read(0x80c6) & 0x02) >> 1);
}

static uint16_t _BStatusRead(void)
{
	uint16_t bstatus = 0;
...
#if 0
static uint32_t bsp_reg_mapping(uint32_t reg)
{
	int i = 0;
	uint32_t reg_map = 0;
...
static void sha_input(sha_t *sha, const uint8_t *data, size_t size)
{
	int i = 0;
	unsigned j = 0;
	int rc = TRUE;
...
static int bsp_hdcp_verify_ksv(const uint8_t *data, size_t size)
{
	size_t i = 0;
	sha_t sha;
...
static void _UpdateKsvListState(uint8_t en)
{
	hdmi_write(0x10010, 0x45);
	hdmi_write(0x10011, 0x45);
	hdmi_write(0x10012, 0x52);
...
static uint8_t bsp_read_ksv_list(void)
{
	int timeout = 1000;
	uint16_t bstatus = 0;
	...
	uint8_t *hdcp_ksv_list_buffer = NULL;/*dev->hdcp.mKsvListBuffer;*/

	hdcp_ksv_list_buffer = kzalloc(sizeof(uint8_t) * 670, GFP_KERNEL);
	...
	if (bsp_hdcp_verify_ksv(hdcp_ksv_list_buffer, size) == TRUE) {
		...
		PRINTF("HDCP_KSV_LIST_READY");
	} else{
		valid = HDCP_ERR_KSV_LIST_NOT_VALID;
		PRINTF("HDCP_ERR_KSV_LIST_NOT_VALID");
	}
...
int bsp_hdmi_hdcp_err_check(void)
{
	...
	if (hdcp_status & 0x80) {
		PRINTF("[%s]:HDCP_ENGAGED interrupt\n", __func__);
