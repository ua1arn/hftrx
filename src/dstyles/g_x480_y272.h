	...
	enum
	{
		...
		BDCV_ALLRX = (20),	// количество ячееек, отведенное под S-метр, панораму, иные отображения
	};
	...
	static const dzone_t dzones [] =
	{
		...
		#if WITHENCODER2
			{	21, 0,	9,	6,	display2_fnblock9,	& dzi_default, PGALL, },	// FUNC menu item label & value
	...
	//		{	26,	16,	3,	3,	display2_agc3,		& dzi_default, PGALL, },	// AGC mode
			...
			{	0,	16,	3,	3,	display2_rec3,		& dzi_default, PGALL, },	// Отображение режима записи аудио фрагмента
			{	0,	16,	3,	3,	display2_mainsub3,	& dzi_default, PGALL, },	// main/sub RX: A/A, A/B, B/A, etc

			{	5,	16,	3,	3,	display2_vfomode3,	& dzi_default, 	PGALL, },	// SPLIT - не очень нужно при наличии индикации на A/B (display2_mainsub3) яркостью.
			{	9,	16,	0,	3,	display2_freqX_b,	& dzi_freqb, 	PGALL, },	// SUB FREQ
			{	21,	16,	3,	3,	display2_mode3_b,	& dzi_modeb,	PGALL, },	// SSB/CW/AM/FM/...

	#if 1
	        {	0,  20,	0,	3,	display2_legend_rx, & dzi_default, PGSWR, },    // Отображение оцифровки шкалы S-метра
	        {	0,  24,	0,	3,	display2_bars_rx,   & dzi_default, PGSWR, },    // S-METER, SWR-METER, POWER-METER
			{	25, 24, 5,	3,	display2_smeors5, 	& dzi_default, PGSWR, },	 // уровень сигнала в баллах S или dBm
	        {	0,  28,	5,	3,	display2_legend_tx, & dzi_default, PGSWR, },    // Отображение оцифровки шкалы PWR & SWR-метра
	        {	0,  32,	0,	1,	display2_bars_tx,   & dzi_default, PGSWR, },    // S-METER, SWR-METER, POWER-METER
...
//			{	0,	28,	0,	BDCV_ALLRX,	display2_combo,	& dzi_default, PGSPE, },// подготовка изображения спектра
			...
			{	0,	51,	5,	3,	display_time5,		& dzi_default, PGALL,	},	// TIME
			{	6, 	51,	3,	3,	display2_atu3,		& dzi_tune, PGALL, },	// TUNER state (optional)
			{	10, 51,	3,	3,	display2_byp3,		& dzi_bypass, PGALL, },	// TUNER BYPASS state (optional)
			{	14, 51,	6,	3,	display2_thermoC,	& dzi_default, PGALL, },	// thermo sensor 20.7C
			{	19, 51,	5,	3,	display2_currlevel, & dzi_default, PGALL, },	// PA drain current d.dd without "A"
			{	25, 51,	5,	3,	display2_voltlevelV, & dzi_voltlevel, PGALL, },	// voltmeter with "V"
		#if WITHAMHIGHKBDADJ
