...
#if defined (DDS1_TYPE) && (DDS1_TYPE == DDS_TYPE_FPGA_NONE)
	// заглушка
	/* параметры входного/выходного адаптеров */
	// IF RX
...
#elif CPUSTYLE_XC7Z || CPUSTYLE_RK356X || CPUSTYLE_BROADCOM
	/* параметры входного/выходного адаптеров */
	// IF RX
	#define WITHADAPTERIFADCWIDTH	32		// 1 бит знак и 31 бит значащих
	#define WITHADAPTERIFADCSHIFT	0		// количество незанятых битов справа.
...
#if CPUSTYLE_XC7Z || CPUSTYLE_RK356X || CPUSTYLE_BROADCOM
...
void board_set_squelch_level(uint_fast8_t v);	/* уровень открывания шумоподавителя */
...
void board_set_skipfilteraf(uint_fast8_t v);
...
int ctcss_squelch(void);
	...
	int dsp_power2y(int_fast16_t power, int ymax, int_fast16_t topdb, int_fast16_t bottomdb);
...
uint_fast8_t dsp_getsmeter(uint_fast8_t * tracemax, uint_fast8_t lower, uint_fast8_t upper);	/* получить значение от АЦП s-метра */
uint_fast16_t dsp_getsmeter10(uint_fast16_t * tracemax, uint_fast16_t lower, uint_fast16_t upper, uint_fast8_t pathi);	/* получить значение от АЦП s-метра */
...
void board_reload_fir(uint_fast8_t ifir, const FLOAT_t * const kf, unsigned Ntap, unsigned CWidth); /* Выдача рассчитанных параметров фильтра в FPGA (симметричные).если апаратура требует только LOCAL обработки, сделать заглушку */
