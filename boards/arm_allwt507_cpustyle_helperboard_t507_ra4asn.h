...
//#define SPI_IOUPDATE_BIT		(UINT32_C(1) << 15)	// * PA15
	...
	#define targetext1		(UINT32_C(1) << 25)		// PD25
	...
	#define SPI_ALLCS_INITIALIZE() do { \
		...
		arm_hardware_piod_outputs(targetext1, 1 * targetext1); /*  */ \
...
#define BOARD_GPIOD_ENC2BTN_BIT (UINT32_C(1) << 21)	// PD21 - second encoder button with pull-up
	...
	#define TARGET_TWI_TWCK_PORT_C(v) do { arm_hardware_pioh_outputs((v), 0); } while (0)
	#define TARGET_TWI_TWCK_PORT_S(v) do { arm_hardware_pioh_inputs(v); } while (0)

	#define TARGET_TWI_TWD		(UINT32_C(1) << 3)
	#define TARGET_TWI_TWD_PIN		(gpioX_getinputs(GPIOH))
	#define TARGET_TWI_TWD_PORT_C(v) do { arm_hardware_pioh_outputs((v), 0); } while (0)
	#define TARGET_TWI_TWD_PORT_S(v) do { arm_hardware_pioh_inputs(v); } while (0)
	...
	*/
	...
	#define BOARD_TCONLCDFREQ (allwnr_t507_get_tcon_lcd0_freq())
		...
		#define BOARD_GT911_RESET_SET(v) do { gpioX_setstate(GPIOD, BOARD_GT911_RESET_PIN, !! (v)); } while (0)
		#define BOARD_GT911_INT_SET(v) do { gpioX_setstate(GPIOD, BOARD_GT911_INT_PIN, !! (v)); } while (0)
		...
		#define BOARD_GT911_RESET_INITIO_2() do { \
			...
			arm_hardware_piod_updown(BOARD_GT911_INT_PIN, 0); \
		} while (0)
