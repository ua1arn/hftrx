...
#ifndef ARM_STM32L051_TQFP32_CPUSTYLE_V1_H_INCLUDED
#define ARM_STM32L051_TQFP32_CPUSTYLE_V1_H_INCLUDED 1
...
#define LS020_RS_INITIALIZE() \
	do { \
		arm_hardware_pioa_outputs2m(LS020_RS, LS020_RS); /* PA15 */ \
	} while (0)
...
//#define SPI_IOUPDATE_BIT		(1U << 15)	// * PA15
	...
	#define SPI_NAEN_PORT_S(v)	do { GPIOB->BSRR = BSRR_S(v); __DSB(); } while (0)
	#define SPI_NAEN_PORT_C(v)	do { GPIOB->BSRR = BSRR_C(v); __DSB(); } while (0)
...
//#endif
	...
	// РїСЂРёСЃРѕРµРґРёРЅРµРЅРёРµ РІС‹РІРѕРґРѕРІ Рє РїРµСЂРёС„РµСЂРёР№РЅРѕРјСѓ СѓСЃС‚СЂРѕР№СЃС‚РІСѓ
		#define	TWIHARD_INITIALIZE() do { \
			enum { WORKMASK = TARGET_TWI_TWCK | TARGET_TWI_TWD };		/* Р±РёС‚РѕРІР°СЏ РјР°СЃРєР°, РѕРїСЂРµРґРµР»СЏРµС‚ РєР°РєРёРј РІС‹РІРѕРґРѕРј С€РµРІРµР»РёС‚СЊ */ \
			arm_hardware_piob_periphopendrain_altfn2(TARGET_TWI_TWCK | TARGET_TWI_TWD, 1);	/* AF=1 */ \
		} while (0) 
	...
	#define HARDWARE_SPI_CONNECT() do { \
			arm_hardware_pioa_updown(SPI_MISO_BIT, 0); /* РїРѕС…РѕР¶Рµ, РЅРµ РёРјРµРµС‚ СЃРјС‹СЃР»Р° */ \
			arm_hardware_pioa_altfn2(SPI_MOSI_BIT | SPI_SCLK_BIT | SPI_MISO_BIT, 0 /* AF_0 */); /* PA5, PA6, PA7. Р’ СЌС‚РёС… РїСЂРѕС†РµСЃСЃРѕСЂР°С… Рё РІС…РѕРґС‹ Рё РІС‹С…РѕРґС‹ РїРµСЂРµРєРґСЋС‡Р°СЋС‚СЃСЏ РЅР° ALT FN */ \
		} while (0)
	...
	#define HARDWARE_SPI_CONNECT_MOSI() do { \
			arm_hardware_pioa_altfn2(SPI_MOSI_BIT, 0 /* AF_0 */);	/* PIO disable for MOSI bit (SD CARD read support) */ \
		} while (0)
