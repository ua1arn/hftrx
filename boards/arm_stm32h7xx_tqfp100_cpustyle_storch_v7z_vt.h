...
#define HARDWARE_SIDETONE_INITIALIZE() do { \
	...
	#define TARGET_TWI_TWCK		(1uL << 6)		// PB6 SCL
	#define TARGET_TWI_TWD		(1uL << 7)		// PB7 SDA
	...
	*/
	...
	#define	HARDWARE_BL_INITIALIZE() do { \
		...
		arm_hardware_piob_outputs(0*(1U << 7), 1 * (1U << 7));		/* PB7 TFT_PWMt */ \
		} while (0)

	/* установка яркости и включение/выключение преобразователя подсветки */
	#define HARDWARE_BL_SET(en, level) do { \
		const portholder_t enmask = 0 * (1U << 7); /* PB7 */ \
		GPIOB->BSRR = \
	...
	#define BOARD_BLINK2_BIT (1uL << 2)	// PA5 - LED2

	#define BOARD_BLINK_INITIALIZE() do { \
		arm_hardware_piod_outputs(BOARD_BLINK_BIT, 0 * BOARD_BLINK_BIT); \
		arm_hardware_pioa_outputs(BOARD_BLINK2_BIT, 0 * BOARD_BLINK2_BIT); \
		} while (0)
	#define BOARD_BLINK_SETSTATE(state) do { \
		...
		if (state) \
			(GPIOA)->BSRR = BSRR_C(BOARD_BLINK2_BIT); \
		else \
			(GPIOA)->BSRR = BSRR_S(BOARD_BLINK2_BIT); \
		} while (0)
