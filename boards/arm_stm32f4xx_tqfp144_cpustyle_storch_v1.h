...
#ifndef ARM_STM32F4XX_TQFP144_CPUSTYLE_STORCH_V1_H_INCLUDED
#define ARM_STM32F4XX_TQFP144_CPUSTYLE_STORCH_V1_H_INCLUDED 1
...
#define LS020_RESET_INITIALIZE() \
	...
	#define ENCODER2_BITS ((1u << 8) | (1u << 7))		// PF8 & PF7
	#define ENCODER2_SHIFT 7
	// Обязательно буфер (входы процессора низковольтные).

	#define ENCODER_INPUT_PORT			(GPIOF->IDR) 
	#define ENCODER_BITS ((1u << 10) | (1u << 9))		// PF10 & PF9
	#define ENCODER_SHIFT 9

	#define ENCODER_INITIALIZE() \
		do { \
			arm_hardware_piof_inputs(ENCODER2_BITS); \
			arm_hardware_piof_updown(ENCODER2_BITS, 0); \
			arm_hardware_piof_onchangeinterrupt(ENCODER2_BITS, ENCODER2_BITS, ENCODER2_BITS, ARM_SYSTEM_PRIORITY); \
			arm_hardware_piof_inputs(ENCODER_BITS); \
			arm_hardware_piof_updown(ENCODER_BITS, 0); \
			arm_hardware_piof_onchangeinterrupt(ENCODER_BITS, ENCODER_BITS, ENCODER_BITS, ARM_SYSTEM_PRIORITY); \
		} while (0)
...
/* Распределение битов в ARM контроллерах */
	...
	#define HARDWARE_SDIOPOWER_C(v)	do { GPIOF->BSRR = BSRR_C(v); __DSB(); } while (0)
	#define HARDWARE_SDIOPOWER_S(v)	do { GPIOF->BSRR = BSRR_S(v); __DSB(); } while (0)
...
#define HARDWARE_SIDETONE_INITIALIZE() do { \
	...
	*/
	...
	#define TARGET_FPGA_OVF_PIN		(GPIOF->IDR)
