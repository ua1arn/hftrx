...
//#define WITHI2SHW	1	/* РСЃРїРѕР»СЊР·РѕРІР°РЅРёРµ I2S - Р°СѓРґРёРѕРєРѕРґРµРє РЅР° I2S2 Рё I2S3	*/
...
//#define SPI_IOUPDATE_BIT		(1U << 15)	// * PA15
	...
	#define HARDWARE_SPI_CONNECT() do { \
			arm_hardware_piob_altfn50(SPI_MOSI_BIT | SPI_MISO_BIT, AF_SPI1); /* Р’ СЌС‚РёС… РїСЂРѕС†РµСЃСЃРѕСЂР°С… Рё РІС…РѕРґС‹ Рё РІС‹С…РѕРґС‹ РїРµСЂРµРєРґСЋС‡Р°СЋС‚СЃСЏ РЅР° ALT FN */ \
			arm_hardware_pioa_altfn50(SPI_SCLK_BIT, AF_SPI1); /* Р’ СЌС‚РёС… РїСЂРѕС†РµСЃСЃРѕСЂР°С… Рё РІС…РѕРґС‹ Рё РІС‹С…РѕРґС‹ РїРµСЂРµРєРґСЋС‡Р°СЋС‚СЃСЏ РЅР° ALT FN */ \
		} while (0)
	...
	#define TARGET_TWI_TWCK_PORT_S(v) do { GPIOF->BSRR = BSRR_S(v); __DSB(); } while (0)
	#define TARGET_TWI_TWD_PORT_C(v) do { GPIOF->BSRR = BSRR_C(v); __DSB(); } while (0)
	#define TARGET_TWI_TWD_PORT_S(v) do { GPIOF->BSRR = BSRR_S(v); __DSB(); } while (0)
	#define TARGET_TWI_TWCK_PIN		(GPIOF->IDR)
	#define TARGET_TWI_TWD_PIN		(GPIOF->IDR)
	#define TARGET_TWI_TWCK		(1u << 0)		// PF0 SCL
	#define TARGET_TWI_TWD		(1u << 1)		// PF1 SDA
	...
	#define	TWISOFT_INITIALIZE() do { \
			arm_hardware_piof_opendrain(TARGET_TWI_TWCK | TARGET_TWI_TWD, TARGET_TWI_TWCK | TARGET_TWI_TWD); \
		} while (0) 
	...
	*/
	...
	#define TARGET_FPGA_OVF_BIT			(1u << 11)	// PF11
