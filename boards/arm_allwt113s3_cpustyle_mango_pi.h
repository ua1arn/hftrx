...
#define TARGET_ENC2BTN_BIT (UINT32_C(1) << 15)	// PE15 - second encoder button with pull-up
	...
	#define FPGA_NCONFIG_BIT		(0 * UINT32_C(1) << 11)	/* PC11 bit conneced to nCONFIG pin ALTERA FPGA */

	/* inputs */
	#define FPGA_CONF_DONE_INPUT	(GPIOC->DATA)
	#define FPGA_CONF_DONE_BIT		(0 * UINT32_C(1) << 10)	/* PC10 bit conneced to CONF_DONE pin ALTERA FPGA */

	#define FPGA_NSTATUS_INPUT		(GPIOC->DATA)
	#define FPGA_NSTATUS_BIT		(0 * UINT32_C(1) << 9)	/* PC9 bit conneced to NSTATUS pin ALTERA FPGA */

	#define FPGA_INIT_DONE_INPUT	(GPIOC->DATA)
	#define FPGA_INIT_DONE_BIT		(0 * UINT32_C(1) << 12)	/* PC12 bit conneced to INIT_DONE pin ALTERA FPGA */
...
#define BOARD_TCONLCDFREQ (allwnrt113_get_tconlcd_freq())
		...
		//#define BOARD_STMPE811_RESET_PIN (UINT32_C(1) << 4)	/* PD4 : tsc/LCD reset, XS26, pin 22 */
