...
#define LS020_RESET_SET(v) do { \
	...
	#define LS020_RESET			(UINT32_C(1) << 4)			// PD4 signal
	...
	#define I2S2HW_SLAVE_INITIALIZE() do { \
			...
			arm_hardware_piob_updown(UINT32_C(1) << 12, 0, 1u << 12); \
			arm_hardware_piob_altfn2m(UINT32_C(1) << 13,	AF_SPI2); /* PB13 I2S2_CK	*/ \
			arm_hardware_piob_updown(UINT32_C(1) << 13, 0, 1u << 13); \
			arm_hardware_piob_altfn2m(UINT32_C(1) << 15,	AF_SPI2); /* PB15 I2S2_SDO - передача */ \
			arm_hardware_piob_updown(UINT32_C(1) << 15, 0, 1u << 15); \
			arm_hardware_piob_altfn2m(UINT32_C(1) << 14,	AF_SPI2); /* PB14 I2S2_SDI, - приём от кодека */ \
			arm_hardware_piob_updown(UINT32_C(1) << 14, 0, 1u << 14); \
		} while (0)
	...
	#define I2S2HW_POOLDOWN() do { \
		arm_hardware_piob_inputs(UINT32_C(1) << 12); /* PB12 I2S2_WS	*/ \
		arm_hardware_piob_updown(UINT32_C(1) << 12, 0, 1u << 12); \
		arm_hardware_piob_inputs(UINT32_C(1) << 13); /* PB13 I2S2_CK	*/ \
		arm_hardware_piob_updown(UINT32_C(1) << 13, 0, 1u << 13); \
		arm_hardware_piob_inputs(UINT32_C(1) << 15); /* PB15 I2S2_SDO - передача */ \
		arm_hardware_piob_updown(UINT32_C(1) << 15, 0, 1u << 15); \
		arm_hardware_piob_inputs(UINT32_C(1) << 14); /* PB14 I2S2_SDI, - приём от кодека */ \
		arm_hardware_piob_updown(UINT32_C(1) << 14, 0, 1u << 14); \
	} while (0)
	...
	#define SAI1HW_INITIALIZE()	do { \
		...
		arm_hardware_pioe_updown(UINT32_C(1) << 3, 1u << 3, 0); \
	} while (0)
#endif /* WITHSAI1HW */
...
/* Распределение битов в ARM контроллерах */
	...
	#define PTT_BIT_PTT					(UINT32_C(1) << 10)		// PD10 - PTT
	#define PTT2_TARGET_PIN				(GPIOD->IDR)
	#define PTT2_BIT_PTT				(UINT32_C(1) << 9)		// PD9 - PTT2
	...
	#define TUNE_BIT_TUNE					(UINT32_C(1) << 11)		// PD11
	...
	#define ELKEY_BIT_LEFT				(UINT32_C(1) << 14)		// PD14
	#define ELKEY_BIT_RIGHT				(UINT32_C(1) << 15)		// PD15
...
//#define SPI_IOUPDATE_BIT		(UINT32_C(1) << 15)	// * PA15
	...
	#define targetnvram		(UINT32_C(1) << 0)		// PE0 nvmem FM25L16B
	#define targetctl1		(UINT32_C(1) << 1)		// PE1 board control registers chain
	#define targetcodec1	(UINT32_C(1) << 2)		// PE2 on-board codec1 NAU8822L
	#define targetadc2		(UINT32_C(1) << 9) 		// PE9 ADC MCP3208-BI/SL chip select (potentiometers)
	#define targetfpga1		(UINT32_C(1) << 10)		// PE10 FPGA control registers CS1
	...
	#define	SPI_SCLK_BIT			(UINT32_C(1) << 3)	// * PB3 бит, через который идет синхронизация SPI
...
#define HARDWARE_UART1_INITIALIZE() do { \
		const uint_fast32_t TXMASK = 0 * (UINT32_C(1) << 11); /* PG11: TX DATA line (2 MHz) */ \
		const uint_fast32_t RXMASK = 0 * (UINT32_C(1) << 2); /* PB2: RX DATA line (2 MHz) - pull-up RX data */  \
...
#define TARGET_ENC2BTN_BIT (UINT32_C(1) << 15)	// PE15 - second encoder button with pull-up
	...
	// I2C1_SDA	PB11
	// I2C1_SCL	PD7
	#define TARGET_TWI_TWCK		(UINT32_C(1) << 7)		// PD7 I2C1_SCL
	...
	#define TARGET_TWI_TWD		(UINT32_C(1) << 11)		// PB11 I2C1_SDA
	...
	#define	TWIHARD_INITIALIZE() do { \
			arm_hardware_piod_periphopendrain_altfn2m(TARGET_TWI_TWCK, AF_I2C1);	/* I2C1_SCL AF=4 */ \
			arm_hardware_piob_periphopendrain_altfn2m(TARGET_TWI_TWD, AF_I2C1);	/* I2C1_SDA AF=4 */ \
		} while (0) 
	...
	*/
	...
	#define TARGET_FPGA_FIR_CS_BIT (UINT32_C(1) << 13)	/* PC13 - fir CS ~FPGA_FIR_CLK */
	...
	#define TARGET_FPGA_FIR1_WE_BIT (UINT32_C(1) << 1)	/* PD1 - fir1 WE */
	...
	#define TARGET_FPGA_FIR2_WE_BIT (UINT32_C(1) << 0)	/* PD0 - fir2 WE */
	...
	#define TARGET_USBFS_VBUSON_BIT (UINT32_C(1) << 2)	// PD2 - нулём включение питания для device
	...
	*/
	...
	#define HARDWARE_LTDC_INITIALIZE(demode) do { \
		...
		/* Bit clock */ \
		arm_hardware_piog_altfn50((UINT32_C(1) << 7), GPIO_AF_LTDC14);		/* CLK PG7 */ \
		/* Control */ \
		...
		/* RED */ \
		arm_hardware_piob_altfn50((UINT32_C(1) << 0), GPIO_AF_LTDC14);		/* PB0 R3 */ \
		arm_hardware_pioa_altfn50((UINT32_C(1) << 11), GPIO_AF_LTDC14);		/* PA11 R4 */ \
		arm_hardware_pioa_altfn50((UINT32_C(1) << 9), GPIO_AF_LTDC14);		/* PA9 R5 */ \
		arm_hardware_pioa_altfn50((UINT32_C(1) << 8), GPIO_AF_LTDC14);		/* PA8 R6 */ \
		arm_hardware_piog_altfn50((UINT32_C(1) << 6), GPIO_AF_LTDC14);		/* PG6 R7 */ \
		/* GREEN */ \
		arm_hardware_pioa_altfn50((UINT32_C(1) << 6), GPIO_AF_LTDC14);		/* PA6 G2 */ \
		arm_hardware_piog_altfn50((UINT32_C(1) << 10), GPIO_AF_LTDC9);		/* PG10 G3 */ \
		arm_hardware_piob_altfn50((UINT32_C(1) << 10), GPIO_AF_LTDC14);		/* PB10 G4 */ \
		arm_hardware_piof_altfn50((UINT32_C(1) << 11), GPIO_AF_LTDC14);		/* PF11 G5 */ \
		arm_hardware_pioc_altfn50((UINT32_C(1) << 7), GPIO_AF_LTDC14);		/* PC7 G6 */ \
		arm_hardware_piog_altfn50((UINT32_C(1) << 8), GPIO_AF_LTDC14);		/* PG8 G7 */ \
		/* BLUE */ \
		arm_hardware_piog_altfn50((UINT32_C(1) << 11), GPIO_AF_LTDC14);		/* PG11 B3 */ \
		arm_hardware_piog_altfn50((UINT32_C(1) << 12), GPIO_AF_LTDC9);		/* PG12 B4 */ \
		arm_hardware_pioa_altfn50((UINT32_C(1) << 3), GPIO_AF_LTDC14);		/* PA3 B5 */ \
	...
	#define HARDWARE_LTDC_SET_DISP(state) do { \
		const uint32_t VSmask = (UINT32_C(1) << 4); 	/* PA4 - VSYNC */ \
		const uint32_t DEmask = (UINT32_C(1) << 13); /* PE13 */ \
	...
	//#define BOARD_BLINK_BIT (UINT32_C(1) << 13)	// PA13 - RED LED LD5 on DK1/DK2 MB1272.pdf
	//#define BOARD_BLINK_BIT (UINT32_C(1) << 14)	// PA14 - GREEN LED LD5 on DK1/DK2 MB1272.pdf
	//#define BOARD_BLINK_BIT (UINT32_C(1) << 14)	// PD14 - LED on small board
	//#define BOARD_BLINK_BIT (UINT32_C(1) << 11)	// PI11 - LED1# on PanGu board
	//#define BOARD_BLINK_BIT (UINT32_C(1) << 11)	// PH6 - LED2# on PanGu board
	...
	#define BOARD_GPIOB_USERBOOT_BIT	(UINT32_C(1) << 1)	/* PB1: ~USER_BOOT */
