...
/* Распределение битов в ARM контроллерах */
	...
	#define	SMHCHARD_CCU_CLK_REG (CCU->SDMMC0_CLK_REG)	/* 0 - SMHC0, 1: SMHC1... */
	#define SMHCHARD_FREQ (allwnr_a64_get_smhc0_freq())
	#define WITHSDHCHW4BIT	1	/* Hardware SD HOST CONTROLLER в 4-bit bus width */
	...
	#define SMHCHARD_FREQ (allwnr_a64_get_smhc1_freq())
	#define WITHSDHCHW4BIT	1	/* Hardware SD HOST CONTROLLER в 4-bit bus width */
	...
	#define SMHCHARD_FREQ (allwnr_a64_get_smhc2_freq())
...
//#define SPI_IOUPDATE_BIT		(UINT32_C(1) << 15)	// * PA15
	...
	// MOSI & SCK port
	#define	SPI_SCLK_BIT			(UINT32_C(1) << 1)	// PD1 SPI1_CLK
	#define	SPI_MOSI_BIT			(UINT32_C(1) << 2)	// PD2 SPI1_MOSI
	#define	SPI_MISO_BIT			(UINT32_C(1) << 3)	// PD3 SPI1_MISO
...
#define TARGET_ENC2BTN_BIT (UINT32_C(1) << 15)	// PE15 - second encoder button with pull-up
	...
	#define	TWIHARD_S_TWI_FREQ (allwnr_a64_get_s_twi_freq()) // APBS2_CLK allwnr_a64_get_apb2_freq() or allwnr_a64_get_apbs2_freq()
#endif
