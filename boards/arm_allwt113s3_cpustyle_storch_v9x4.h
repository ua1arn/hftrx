...
#ifndef ARM_ALWT113S3_CPUSTYLE_STORCH_V9X4_H_INCLUDED
#define ARM_ALWT113S3_CPUSTYLE_STORCH_V9X4_H_INCLUDED 1
...
#define WITHDEBUG_UART0	1
	...
	#define WITHBLPWMCTL	1		// Имеется управление яркостью подсветки дисплея через PWM
		...
		#define WITHFPGAPIPE_RTS96 WITHRTS96	/* в том же фрейме передаются квадратуры RTS96 */
		#define WITHFPGAPIPE_RTS192 WITHRTS192	/* в том же фрейме передаются квадратуры RTS192 */
		...
		//#define WITHCODEC1_DMIC_RX_MASTER	1	/* The DMIC controller - digital microphone interface */
...
//	#define SPI_TARGET_MISO_PIN		gpioX_getinputs(GPIOC)
	...
	#define	SPIDFHARD_PTR SPIHARD_PTR	/* 0 - SPI0, 1: SPI1... */
...
#define HARDWARE_UART2_INITIALIZE() do { \
	...
	*/
	...
	#define HARDWARE_LTDC_INITIALIZE(demode) do { \
		...
		const portholder_t MODEmask = (UINT32_C(1) << 0); 	/* PD0 LCD MODE */ \
		/* set LCD DE/SYNC mode */ \
		arm_hardware_piod_outputs(MODEmask, ((demode) != 0) * MODEmask);	/* PD0 = LCD MODE */ \
...
#endif /* ARM_ALWT113S3_CPUSTYLE_STORCH_V9X4_H_INCLUDED */
