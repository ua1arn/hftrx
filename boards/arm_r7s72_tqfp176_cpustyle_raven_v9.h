...
#define LS020_RS_INITIALIZE() \
	do { \
		arm_hardware_pio3_outputs(WRITEE_BIT, 1 * WRITEE_BIT); /* в новой версии платы выдавать "0" */ \
...
#define LS020_RESET_INITIALIZE() \
	...
	// P2_6 теперь используется как сигнал от правого контакта ключа ELKEY_BIT_RIGHT
	...
	//#define ENCODER_SHIFT 6	// Отсутствие этого значения означает что биты не подряд
	...
	#define ENCODER_INITIALIZE() \
		do { \
			...
			arm_hardware_irqn_interrupt(1, 3, ARM_SYSTEM_PRIORITY, spool_encinterrupt); /* IRQ1, both edges */ \
			arm_hardware_irqn_interrupt(2, 3, ARM_SYSTEM_PRIORITY, spool_encinterrupt); /* IRQ2, both edges */ \
		} while (0)
...
/* Распределение битов в ARM контроллерах */
	...
	#define ELKEY_BIT_RIGHT				(1U << 6)	// P2_6 (пренесено с ошибочно присоединенного вывода 37)
...
#define	SPI_MISO_BIT			(1U << 3)	// * P6_3 бит, через который идет ввод с SPI.
	...
	#define TARGET_FPGA_FIR_CLK_PORT_C(v)	do { R7S721_TARGET_PORT_C(5, v); } while (0)
	#define TARGET_FPGA_FIR_CLK_PORT_S(v)	do { R7S721_TARGET_PORT_S(5, v); } while (0)
	#define TARGET_FPGA_FIR_CLK_BIT (1U << 15)	/* P5_15 - fir clock */
	...
	#define TARGET_FPGA_FIR_INITIALIZE() do { \
				...
				arm_hardware_pio5_outputs(TARGET_FPGA_FIR_CLK_BIT, TARGET_FPGA_FIR_CLK_BIT); \
			} while (0)
#endif /* WITHDSPEXTFIR */
