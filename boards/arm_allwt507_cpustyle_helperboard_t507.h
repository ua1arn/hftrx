...
#define LS020_RESET_INITIALIZE() do { \
	...
	#define BOARD_GPIOA_ENCODER_BITS		(ENCODER_BITA | ENCODER_BITB)
	#define BOARD_GPIOA_ENCODER2_BITS		(ENCODER2_BITA | ENCODER2_BITB)
	...
	#define ENCODER_INITIALIZE() do { \
			...
			einthandler_initialize(& h1, BOARD_GPIOA_ENCODER_BITS, spool_encinterrupts, & encoder1); \
			...
			einthandler_initialize(& h2, 0*BOARD_GPIOA_ENCODER2_BITS, spool_encinterrupts, & encoder2); \
...
#define BOARD_GPIOA_ENC2BTN_BIT (UINT32_C(1) << 8)	// PA8 - second encoder button with pull-up
	...
	*/
	...
	#define	HARDWARE_BL_INITIALIZE() do { \
		...
		arm_hardware_pioa_opendrain(BLPinMSB, 0 * BLPinMSB); /* максимальный ток */ \
		arm_hardware_pioa_opendrain(BLPinLSB, 0 * BLPinLSB); /* максимальный ток */ \
	} while (0)
	...
	#define	TCONLCD_IX 0	/* 0 - TCON_LCD0, 1: TCON_LCD1, 2: TCON_TV0, 3: TCON_TV1 */
	#define	TCONLCD_PTR TCON_LCD0	/* 0 - TCON_LCD0, 1: TCON_LCD1, 2: TCON_TV0, 3: TCON_TV1 */
	#define	TCONLCD_CCU_CLK_REG (CCU->TCON_LCD0_CLK_REG)	/* 0 - TCON_LCD0, 1: TCON_LCD1, 2: TCON_TV0, 3: TCON_TV1 */
